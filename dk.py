...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QTextEdit, QTabWidget, QStyledItemDelegate, QMessageBox
)
...
def tongji(d,serial,dir_path):
    ...
    while (True):
        if (d(resourceId='com.alipay.mobile.socialwidget:id/red_dot_alert').exists(timeout=3) or (flag == 1)):
            elements = d(resourceId='com.alipay.mobile.socialwidget:id/red_dot_alert')  # 获取所有文本为'some_text'的元素
            ...
            if (d(resourceId='com.alipay.mobile.antui:id/title_text').exists(timeout=3)):
                phone_num = ""
                title_text = d(resourceId='com.alipay.mobile.antui:id/title_text').get_text()
                if (title_text == None):
                    break
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.caozuo_config.setStyleSheet("""  
                                ...
                                """)
        self.config_area = QLabel("*" * 55 + "配置区" + "*" * 55)
        self.config_area.setFixedHeight(50)
        self.config_area.setStyleSheet("""  
                                        ...
                                        """)
        self.config_area_douyin = QLabel("*" * 55 + "配置区" + "*" * 55)
        self.config_area_douyin.setFixedHeight(50)
        self.config_area_douyin.setStyleSheet("""  
                                                    QLabel {  
                                                        font-size: 14px; /* 字体大小 */  
                                                        font-weight: bold; /* 字体加粗 */
                                                        font-family: "Arial", sans-serif; /* 字体家族，使用Arial或系统默认的无衬线字体 */  
        ...
        self.radio_button1 = QCheckBox("抖音关注")
        self.radio_button1.setChecked(True)
        self.radio_button2 = QCheckBox("抖音私信")
        self.radio_button3 = QCheckBox("快手关注")
        self.radio_button3.setChecked(True)
        self.radio_button4 = QCheckBox("快手私信")
        self.radio_button4.setChecked(True)
        self.radio_button6 = QCheckBox("养号")
        ...
        #以下是养号设置
        self.label_from_yanghao = QLabel('                          养号点赞概率：')
        self.line_edit_from_yanghao = QLineEdit("8")
        self.line_edit_from_yanghao.setFixedWidth(40)

        self.label_to_yanghao = QLabel('%')

        self.label_from111_yanghao = QLabel('            养号收藏概率:')
        self.jiarenshurukuang_yanghao = QLineEdit("30")
        self.jiarenshurukuang_yanghao.setFixedWidth(40)
        self.label_from222_yanghao = QLabel('%                      ')
        # 使用 QHBoxLayout 将 "加人间隔：" 输入框 和 "至" 组合在一起
        self.h_layout_yanghao = QHBoxLayout()
        self.h_layout_yanghao.addWidget(self.label_from_yanghao)
        self.h_layout_yanghao.addWidget(self.line_edit_from_yanghao)
        self.h_layout_yanghao.addWidget(self.label_to_yanghao)
        self.h_layout_yanghao.addWidget(self.label_from111_yanghao)
        self.h_layout_yanghao.addWidget(self.jiarenshurukuang_yanghao)
        self.h_layout_yanghao.addWidget(self.label_from222_yanghao)
        ...
        layout.addLayout(self.h_layout_yanghao)
        ...
        self.tab_widget.addTab(central_widget, '手机列表')
        ...
        # self.tab_widget.addTab(tab2, '抖音私信')
        #
        # tab3 = QWidget()
        ...
        # self.tab_widget.addTab(tab3, '快手私信')
        ...
        self.table_widget_douyin = QTableWidget(self)
        self.initialize_table_structure()  # 初始化表格结构
        self.table_widget_douyin.setItemDelegate(MyItemDelegate(self))  # 可选：自定义委托以处理空值显示
        self.main_layout_douyin.addWidget(self.table_widget_douyin)
        #self.table_widget_douyin.setColumnWidth(1, 80)
        self.table_widget_douyin.setFixedHeight(400)
        ...
        self.main_layout_douyin.addWidget(self.config_area_douyin)
        self.file_button_douyin.clicked.connect(self.showDialog)

        checkbox_layout = QHBoxLayout()
        self.checkbox1 = QLineEdit("给力给力", self)
        self.checkbox1.setFixedHeight(60)
        self.checkbox1.setFixedWidth(100)
        self.checkbox2 = QCheckBox("开启发送私信功能", self)
        self.checkbox3 = QCheckBox("开启关注私信用户", self)

        self.label_from_douyin = QLabel('私信间隔')
        self.line_edit_from_douyin = QLineEdit("8")
        self.line_edit_from_douyin.setFixedWidth(40)
        self.label_to = QLabel('至')
        self.line_edit_end_douyin = QLineEdit("40")
        self.line_edit_end_douyin.setFixedWidth(40)
        self.label_miao_douyin = QLabel('秒')

        checkbox_layout.addWidget(self.checkbox1)
        checkbox_layout.addWidget(self.checkbox2)
        checkbox_layout.addWidget(self.checkbox3)
        checkbox_layout.addWidget(self.label_from_douyin)
        checkbox_layout.addWidget(self.line_edit_from_douyin)
        checkbox_layout.addWidget(self.label_to)
        checkbox_layout.addWidget(self.line_edit_end_douyin)
        checkbox_layout.addWidget(self.label_miao_douyin)
        checkbox_layout.addStretch()  # 添加一个伸缩因子以填充剩余空间

        self.main_layout_douyin.addLayout(checkbox_layout)
        ...
        self.tab_widget.addTab(container, '抖音私信')

        # 下面是快手的buju
        self.excel_file_kuaishou = None
        self.data_kuaishou = pd.DataFrame()
        self.update_interval_kuaishou = 3000  # 更新间隔，单位为毫秒
        self.timer_kuaishou = QTimer(self)
        self.timer_kuaishou.timeout.connect(self.read_excel_and_update_table_kuaishou)

        # self.setWindowTitle('Excel Data Updater')
        # self.setGeometry(100, 100, 800, 600)
        # 主布局

        self.main_layout_kuaishou = QVBoxLayout()
        self.table_widget_kuaishou = QTableWidget(self)
        self.initialize_table_structure_kuaishou()  # 初始化表格结构
        self.table_widget_kuaishou.setItemDelegate(MyItemDelegate(self))  # 可选：自定义委托以处理空值显示
        self.main_layout_kuaishou.addWidget(self.table_widget_kuaishou)
        # self.table_widget_douyin.setColumnWidth(1, 80)
        self.table_widget_kuaishou.setFixedHeight(400)

        self.h_layout_dir_kuaishou = QHBoxLayout()
        self.file_textbox_kuaishou = QLineEdit("请输入文件夹路径")
        self.h_layout_dir_kuaishou.addWidget(self.file_textbox_kuaishou)
        self.file_button_kuaishou = QPushButton("选择文件", self)
        self.temp_kuaishou = QLabel("                          ")
        self.h_layout_dir_kuaishou.addWidget(self.file_button_kuaishou)
        self.main_layout_kuaishou.addLayout(self.h_layout_dir_kuaishou)
        self.main_layout_kuaishou.addWidget(self.config_area)
        self.file_button_kuaishou.clicked.connect(self.showDialog_kuaishou)

        checkbox_layout_kuaishou = QHBoxLayout()
        self.checkbox1_kuaishou = QLineEdit("给力给力", self)
        self.checkbox1_kuaishou.setFixedHeight(60)
        self.checkbox1_kuaishou.setFixedWidth(100)
        self.checkbox2_kuaishou = QCheckBox("开启发送私信功能", self)
        self.checkbox3_kuaishou = QCheckBox("开启关注私信用户", self)

        self.label_from_kuaishou = QLabel('私信间隔')
        self.line_edit_from_kuaishou = QLineEdit("8")
        self.line_edit_from_kuaishou.setFixedWidth(40)
        self.label_to_kuaishou = QLabel('至')
        self.line_edit_end_kuaishou = QLineEdit("40")
        self.line_edit_end_kuaishou.setFixedWidth(40)
        self.label_miao_kuaishou = QLabel('秒')

        checkbox_layout_kuaishou.addWidget(self.checkbox1_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.checkbox2_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.checkbox3_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.label_from_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.line_edit_from_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.label_to_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.line_edit_end_kuaishou)
        checkbox_layout_kuaishou.addWidget(self.label_miao_kuaishou)
        checkbox_layout_kuaishou.addStretch()  # 添加一个伸缩因子以填充剩余空间

        self.main_layout_kuaishou.addLayout(checkbox_layout_kuaishou)

        container_kuaishou = QWidget(self)
        container_kuaishou.setLayout(self.main_layout_kuaishou)
        self.timer_kuaishou.start(self.update_interval_kuaishou)
        self.tab_widget.addTab(container_kuaishou, '快手私信')

    def initialize_table_structure(self):
        # 假设初始的列头是 ['编号', 'uid', '私信状态', '关注状态', '任务倒计时']
        self.table_widget_douyin.setColumnCount(5)
        self.table_widget_douyin.setHorizontalHeaderLabels(['编号', 'uid', '私信状态', '关注状态', '任务倒计时'])
    def initialize_table_structure_kuaishou(self):
        # 假设初始的列头是 ['编号', 'uid', '私信状态', '关注状态', '任务倒计时']
        self.table_widget_kuaishou.setColumnCount(5)
        self.table_widget_kuaishou.setHorizontalHeaderLabels(['编号', 'uid', '私信状态', '关注状态', '任务倒计时'])
    ...
    def read_excel_and_update_table_kuaishou(self):
        print("fudai_path")
        if self.excel_file:
            try:
                print("222")
    ...
    def update_table_structure(self):
        ...
        self.table_widget_douyin.setHorizontalHeaderLabels(self.data.columns)
        self.table_widget_douyin.setColumnCount(len(self.data.columns))
        # 根据数据框的行数设置表格的行数
        self.table_widget_douyin.setRowCount(len(self.data))
    ...
    def update_table_data(self):
        ...
        self.table_widget_douyin.setRowCount(row_count)  # +1 是为了包含标题行
        ...
        for row_idx, row_data in self.data.iterrows():
            ...
            for col_idx, value in enumerate(row_data):
                ...
                self.table_widget_douyin.setItem(row_in_table, col_idx, item)
        for col_idx in range(self.table_widget_douyin.columnCount()):
            self.table_widget_douyin.setColumnWidth(col_idx, 150)
