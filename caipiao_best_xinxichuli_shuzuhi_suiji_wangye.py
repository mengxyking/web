...
def extract_continuous_single_double_prefix(text):
    ...
    """
    # 正则规则：匹配开头1个或多个连续的“单”或“双”
    # ^ 锚定开头，(单|双)+ 匹配1个或多个连续的单/双
    match = re.match(r'^(单|双)+', text)

    if match:
        # 返回匹配到的连续单/双组合（如“双双”“单单”）
        return match.group()
    else:
        # 未匹配到返回空字符串，便于后续判断
        return ""
...
def extract_number_from_string(text):
    ...
    # 2. 处理匹配结果
    if number_matches:
        # 取第一个匹配到的数字（适配"双52"这类只有一个数字的场景）
        # 若需要提取所有数字，可返回 [int(num) for num in number_matches]
...
class LotteryLogic:
    ...
    def generate_bet_sequence(self, danshuang1, danshuang2, danshuang3, danshuang4, danshuang5, danshuang6):
        ...
        """
        try:
            # 拆分各单双配置为列表，并过滤空值和非单/双的内容
            def parse_seq(seq_str):
                """辅助函数：解析单双序列字符串，处理空值"""
                if not seq_str or seq_str.strip() == "":  # 处理空字符串情况
                    return []
                return [p.strip() for p in seq_str.split(',') if p.strip() in ["单", "双"]]
