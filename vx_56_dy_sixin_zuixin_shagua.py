...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifousixinzuozhe,shifousixinpinglunyonghu,shifouzuixin,jinyiri,jinqiri,shifouqufnawenzi,shifouqunfashoucang,qunfaneirong,qunfajiangeshijian,shifoufbudouyin,douyinfabuneirong,douyinfabushijianjiange,douyinsixinneirong,douyinsixinguanjianzi,douyinsixinshijianjiange,douyinsixingeshuxianzhi,phone_len,phone_count,shifoudouyinsixin,douyinsixinwenjian):
...
def douyinsixin(d, serial,neirong,guanjianzi,douyinsixinshijianjiange,douyinsixingeshuxianzhi):
    ...
    while(sixin_count < int(douyinsixingeshuxianzhi)):
        ...
        for user in users:
            print(user.info)
            #if((str(user.info).count("粉丝")<1) and (str(user.info).count("抖音号")<1) and (str(user.info).count("直播中")<1)):
            yy = user.info["bounds"]["top"]
            if(yy):
                print("int(yy)=",int(yy))
                print("bar_bottom=",bar_bottom)
                print(int(yy) - bar_bottom)
                if (int(yy) - int(bar_bottom) > 100):
                    d.click(300,int(yy))
                    time.sleep(3)

                    if (d(textContains="抖音号：").exists(timeout=3)):
                        match = re.search(r'抖音号：([^\s，。、]+)', str(d(textContains="抖音号：").get_text()))
                        if match:
                            douyin_id = match.group(1)
                            print("提取的抖音号是:", douyin_id)
                            ...
                            if (not os.path.isfile(file_path)):
                                ...
                                time.sleep(2)
                                result_sixin_douyin = dysixin(d, serial, neirong)
                                if(result_sixin_douyin == 99):
                                    return 99
                                time.sleep(
                                    random.randint(int(douyinsixinshijianjiange), int(douyinsixinshijianjiange) + 8))
                    else:
                        print("22")
...
def dysixin(d,serier,neirong):
    ...
    if (d(description="给陌生人发送消息").exists(timeout=1)):
        print("发送消息")
        #d(description="发送").click()
        #random_click_view(d,d(description="发送").info)
        return 99
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,shifousixinzuozhe,shifousixinpinglunyonghu,shifouzuixin,jinyiri,jinqiri,shifouqufnawenzi,shifouqunfashoucang,qunfaneirong,qunfajiangeshijian,shifoufbudouyin,douyinfabuneirong,douyinfabushijianjiange,douyinsixinneirong,douyinsixinguanjianzi,douyinsixinshijianjiange,douyinsixingeshuxianzhi,phone_len,phone_count,shifoudouyinsixin,douyinsixinwenjian):
    ...
    if(shifoudouyinsixin):
        contents = []
        # 打开文件（默认以只读模式）
        with open(douyinsixinwenjian, 'r', encoding='utf-8') as file:
        ...
        neirongs = contents
        guanjianzis = contents
        dijige_neirong = 0
        neirongs_len = len(neirongs)

        if (neirongs_len < phone_len):
            dijige_neirong = phone_count % neirongs_len
        else:
            dijige_neirong = phone_count

        dijige_guanjianzi = 0
        neiguanjianzis_len = len(guanjianzis)

        if (neiguanjianzis_len < phone_len):
            dijige_guanjianzi = phone_count % neiguanjianzis_len
        else:
            dijige_guanjianzi = phone_count

        if(str(neirongs[dijige_neirong]).count("-") < 1):
            print("私信关键字配置不对")
            return

        sixin_guanjianzi = str(neirongs[dijige_neirong]).split("-")
        updata_pkl("./shuju/" + serial + ".pkl", "执行状态", "运行中")
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "dy私信任务")
        result_xx = douyinsixin(d,serial,sixin_guanjianzi[1],sixin_guanjianzi[0],douyinsixinshijianjiange,douyinsixingeshuxianzhi)
        if(result_xx == 99):
            return 99
    ...
    if(shifouqunfashoucang == True or shifouqufnawenzi == True):
        ...
        time.sleep(0.5)
        updata_pkl("./shuju/" + serial + ".pkl", "执行状态", "运行中")
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "vx群发任务")
        ...
        result_qunfa = qunfa(d, serial,shifouqufnawenzi,shifouqunfashoucang,qunfaneirong,qunfajiangeshijian)
        if(result_qunfa == 99):
            return 99
        if(result_qunfa == 0 or result_qunfa == None):
            return
    if(shifoufbudouyin):
        videos = getCount_videos(d)

        if(videos > 0):

            count_pub_ = 0
            while(count_pub_ < videos):
                douyin_public(d,count_pub_,douyinfabuneirong)
                count_pub_ += 1
                time.sleep(int(douyinfabushijianjiange))
...
def backToHome(d):
    ...
    while(dd < 10):
        ...
        if(len(elements)>0):
            return "1"
        elements_save = d(text='不保存返回')  # 获取所有文本为'some_text'的元素
        # print(len(elements))
        if (len(elements_save) > 0):
            elements_save.click()
            time.sleep(2)
        time.sleep(0.8)
        d.press("back")
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.radio_button10 = QCheckBox("抖音发布")
        ...
        self.radio_button2_1 = QCheckBox("抖音私信")
        self.radio_button2_1.setChecked(False)
        # self.radio_button2_8 = QCheckBox("群发文字")
        # self.radio_button2_8.setChecked(False)
        # self.radio_button2_10 = QCheckBox("抖音发布")
        ...
        self.label_from111_shoucang_diwuhang = QLabel('抖音发布文案：')
        self.label_from111_shoucang_diwuhang.setFixedWidth(80)
        ...
        if (douyinfabushuliang != None):
            ...
            self.douyinfabushuliang = QLineEdit("请输入群发内容")
        self.douyinfabushuliang.setFixedWidth(250)
        self.label_from222_shoucang_diwuhang = QLabel('')
        self.label_from222_pinglun_diwuhang = QLabel('        ')

        self.label_from111_dianzan_diwuhang = QLabel('抖音间隔时间：')
        self.label_from111_dianzan_diwuhang.setFixedWidth(80)
        ...
        self.qunfashijianjiange.setFixedWidth(50)
        self.label_from222_dianzan_diwuhang = QLabel('秒     ')
    ...
    def execute_button_clicked(self):
        ...
        updata_pkl_config_mianban("qunfashijiajiange", self.qunfashijianjiange.text())
    ...
    def thread_temp(self,tasks):
        phone_len = len(self.selected_ids)
        count_phone = 0
        for serial in self.selected_ids:
            print("fudai_path")
            thread = threading.Thread(target=operate_device, args=(serial,self.file_textbox.text(),self.file_textbox_comment.text(),tasks,self.run_time.text(),self.line_edit_from_search.text(),self.line_edit_to_search.text(),self.line_edit_from.text(),self.line_edit_to.text(),self.jiarenshurukuang.text(),self.shoucang_gailv.text(),"shoucangpinglun",self.shoucang_dianzan.text(),"shoucangdianzan",self.huadongcishu_big.text(),self.file_textbox_gouwu.text(),self.radio_button2.isChecked(),self.radio_button3.isChecked(),self.radio_button4.isChecked(),self.radio_button6.isChecked(),self.radio_button7.isChecked(),self.radio_button8.isChecked(),self.radio_button9.isChecked(),self.shoucang_gailv.text(),self.shoucang_dianzan.text(),self.radio_button10.isChecked(),self.douyinfabushuliang.text(),self.qunfashijianjiange.text(),self.douyinsixinneirong.text(),self.douyinsixinguanjianzi.text(),self.douyinsixinsixinshijianjiange.text(),self.douyinsixinsixingeshuxianzhi.text(),phone_len,count_phone,self.radio_button2_1.isChecked(),self.file_textbox_gouwu.text()))
