...
import time
import uuid
import jingdong_daoxu_aozhou_feiting888_huan_feiting
...
def updata_pkl(key,value):
    pklfile = "shuju_aozhou_feiting_huan.pkl"
...
def run_script():
    ...
    try:
        ...
        for temp in shuzu:
            print(temp)
        print("开始执行脚本")
        jingdong_daoxu_aozhou_feiting888_huan_feiting.main_gui(file_textbox.text(),start_textbox_kami.text(),shuzu,file_textbox_tz_dizhi.text(),file_textbox_jiekou_dizhi.text(),"",touzhu_zhengfan.currentText())
...
app = QApplication([])
window = QWidget()
window.setWindowTitle("模拟测试飞艇跟反往期")
