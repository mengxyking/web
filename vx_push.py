...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(['选中', '编号', '昵称','连接状态', '运行状态','当前任务',"加好友统计"])
        ...
        # 添加选项到 QComboBox
        self.combo_box.addItem('瓷砖视频')
        self.combo_box.addItem('朋友圈视频')
        self.combo_box.addItem('短视频短剧')

        self.radio_button1 = QCheckBox("往手机推送")
        self.radio_button1.setChecked(True)
        self.radio_button2 = QCheckBox("微信养号")
        self.radio_button2.setChecked(True)
        self.radio_button3 = QCheckBox("养号是否刷朋友圈")
        ...
        self.horizontal_layout.addWidget(self.radio_button15)
        ...
        self.label_file = QLabel("                          请选择本机文件目录:")
        ...
        if(file_temp_path != None):
            ...
            self.file_textbox = QLineEdit("请选择本机文件目录")
        self.h_layout_dir.addWidget(self.file_textbox)
        self.file_button = QPushButton("选择文件夹",self)
        self.temp = QLabel("                          ")
        ...
        self.label_file_comment = QLabel("                          请输入手机文件夹目录:")
        self.h_layout_dir_comment.addWidget(self.label_file_comment)

        file_temp_path_comment = get_value_by_key_pkl("shuju_config.pkl", "shouji_wenjianjia_mulu")
        if (file_temp_path_comment != None):
            self.file_textbox_comment = QLineEdit(file_temp_path_comment)
        else:
            self.file_textbox_comment = QLineEdit("请输入手机文件夹目录")
        ...
        # 以下是评论文件选择器
        self.h_layout_dir_comment_name = QHBoxLayout()
        self.label_file_comment_name = QLabel("                          请输入文件名:")
        self.h_layout_dir_comment_name.addWidget(self.label_file_comment_name)

        file_temp_path_comment_name = get_value_by_key_pkl("shuju_config.pkl", "shouji_wenjianjia_mulu_name")
        if (file_temp_path_comment_name != None):
            self.file_textbox_comment_name = QLineEdit(file_temp_path_comment_name)
        else:
            self.file_textbox_comment_name = QLineEdit("请输入文件名")
        self.h_layout_dir_comment_name.addWidget(self.file_textbox_comment_name)
        self.temp_comment_name = QLabel("                          ")
        # self.h_layout_dir_comment.addWidget(self.file_button_comment)
        self.h_layout_dir_comment_name.addWidget(self.temp_comment_name)
        ...
        layout.addLayout(self.h_layout_dir_comment_name)
    ...
    def showDialog(self):
        ...
        if dialog.exec() == QFileDialog.DialogCode.Accepted:
            ...
            if selected_files:
                ...
                updata_pkl_config_mianban("benji_mulu", selected_file)
    ...
    def execute_button_clicked(self):
        ...
        updata_pkl_config_mianban("shouji_wenjianjia_mulu", self.file_textbox_comment.text())
        print("---",self.file_textbox_comment_name.text())
        updata_pkl_config_mianban("shouji_wenjianjia_mulu_name", self.file_textbox_comment_name.text())
    ...
    def thread_temp(self,tasks):
        for serial in self.selected_ids:
            thread = threading.Thread(target=operate_device, args=(serial,self.file_textbox.text(),self.file_textbox_comment.text()))
    ...
    def execute_delete_button_clicked(self):
        ...
        for temp in self.selected_ids:
            print(temp)
            class_phone = get_value_by_key_pkl("config.pkl", temp)
            print(temp,class_phone,self.file_textbox.text())

            thread = threading.Thread(target=self.creat_video,
                                      args=(temp,class_phone,self.file_textbox.text(),i))
            thread.start()
            i += 1
            #time.sleep(random.randint(2, 5))
        self.refresh_pkl_files()
        self.selected_ids = []
    def creat_video(self,phone,nick_name,file_path,i):
        file_name_leibie = {"pengyouquan":"pengyouquan","duanshipin":"duanshipin","cizhuan":"cizhuang"}
        leibie = "pengyouquan"
        if(self.combo_box.currentText() == "朋友圈视频" ):
            leibie = "pengyouquan"
        elif(self.combo_box.currentText() == "短视频短剧"):
            leibie = "duanshipin"
        else:
            leibie = "cizhuan"
        filepath = './shuju/' + phone + ".pkl"
        if (os.path.isfile(filepath)):
            updata_pkl(filepath, "执行状态", "任务开始")

        print("----------->",i)
        time.sleep(i*3)
        if(str(nick_name).startswith("0")):
            nick_name=nick_name[1:]
            print("nick_name----",nick_name)
        print(phone,nick_name,file_path)
        ...
        key_name = find_file_with_matching_number(file_path,nick_name)
        if(key_name == None):
            print("weikong")
            return

        print("key_name----",key_name)
        key_name_path = file_path + "/" + key_name

        path_phone = file_path+"/"+phone
        ensure_folder_exists(path_phone,False)
        path_phone_leibie = file_name_leibie[leibie]
        path_phone_leibie_path = path_phone + "/" + path_phone_leibie
        ensure_folder_exists(path_phone_leibie_path)

        file_temp = path_phone_leibie_path+"/"+str(self.file_textbox_comment_name.text())+".mp4"
        filepath = './shuju/' + phone + ".pkl"
        if (os.path.isfile(filepath)):
            updata_pkl(filepath, "执行状态", "开始复制文件")
        shutil.copy2(key_name_path, file_temp)

        filepath = './shuju/' + phone + ".pkl"
        if (os.path.isfile(filepath)):
            updata_pkl(filepath, "执行状态", "复制完成,开始推送")

        sdcard_path = "sdcard/"+self.file_textbox_comment.text()
        cmd_mkdir = "adb -s "+phone + " shell mkdir -p "+sdcard_path
        print("cmd_mkdir=",cmd_mkdir)
        shell_neibu(cmd_mkdir)

        cmd = "adb -s "+phone + " push " + file_temp +" "+sdcard_path
        print("cmd=", cmd)
        shell_neibu(cmd)

        filepath = './shuju/' + phone + ".pkl"
        if (os.path.isfile(filepath)):
            updata_pkl(filepath, "执行状态", "推送完成")
...
def copy_file(a_path, b_path):
    ...
    print(f"文件已从 {a_path} 复制到 {b_path}")

def ensure_folder_exists(folder_path,is_shanchu = True):
    # 如果文件夹存在，则删除它及其内容
...
def find_file_with_matching_number(folder_path, number_str):
    # 正则表达式模式，用于匹配文件名中的括号内容
