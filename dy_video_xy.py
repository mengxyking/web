...
def operate_device(serial, dir_path, task,big,small,jiarenshuliang):
    ...
    # time.sleep(3)
    try:
        bbb(d, serial, jiarenshuliang,big)
    except BaseException as ee:
        print("崩溃了",ee)
        bbb(d, serial, jiarenshuliang,big)
def bbb(d,serial,jiarenshuliang,big):
    kill_count = 0
    ...
    while (True):
        ...
        #     d(text='打开看看').click()
        shell_neibu(
            f"adb -s {serial} shell am start -a android.intent.action.VIEW -d  {url}  -n com.sljh.uabrowser/com.cy.browser.BrowserActivity")
        time.sleep(3)
