        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                if(str(line).count("@")>0):
                    ...
                    if(str(phone_num).count("@")>0):#当前 用户列表里面是邮箱
                        if(str(search_string) == temp1[1]):
                            return line.strip()
                    elif(not str(temp1[0]) == "***"):
                        print("temp1=", temp1)
                        print("temp1[0][0:3]=", temp1[0][0:3])
                        print("phone_num[0:3]=", phone_num[0:3])
                        print("str(temp1[2]=", str(temp1[2]))
                        ...
                        if ((temp1[2][0:3] == phone_num[0:3]) and(temp1[2][-2:] == phone_num[-2:]) and (str(temp1[1]).count(search_string) > 0)):
                            return line.strip()
...
def tongji_temp(d,serial,dir_path):
    ...
    if (d(resourceId='com.alipay.android.phone.wallet.profileapp:id/display_name').exists(timeout=3)):
        title_text = d(resourceId='com.alipay.android.phone.wallet.profileapp:id/display_name').get_text()
        print("title_text=",title_text)
        if (title_text == None):
            return

        ee = d(resourceId="com.alipay.android.phone.wallet.profileapp:id/alipay_account")
        if (ee):
            phone_num = str(ee.get_text()[6:])
            time.sleep(1)
...
def addFriend(d,no,task,serial):
    ...
    if (d(resourceId="com.alipay.android.phone.wallet.profileapp:id/tv_right").exists(timeout=3)):
        nick_name_temp = d(resourceId="com.alipay.android.phone.wallet.profileapp:id/tv_right").get_text()
    ...
    if (d(resourceId="com.alipay.android.phone.wallet.profileapp:id/tv_name").exists(timeout=3)):
        account_name = d(resourceId="com.alipay.android.phone.wallet.profileapp:id/tv_name").get_text()  # 获取当前支付宝账号的名字
