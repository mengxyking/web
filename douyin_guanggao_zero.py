...
def main_control(service):
    ...
    #
    # return "88"
    result_main = main_douyin(service)
...
def xhs_tongcheng(serial,d):#tongchengguanjianzi_xhs
    ...
    try:
        ...
        print("完整错误信息:")
        print(error_info)
def back_to_live_room(d):
    dd = 0
    time.sleep(1)
    while (dd < 8):
        elements33 = d(text='说点什么...')
        elements3444 = d(text='直播已结束')
        # print(len(elements))
        if (len(elements33) > 0):
            return "1"
        if (len(elements3444) > 0):
            return "2"
        # time.sleep(1.5)
        d.press("back")
        print("222")
        dd += 1
        time.sleep(1)
def main_douyin(service):
    d = get_device(service)
    for i in range(99):
        print(f"-----------{i}------------")
        ...
        if (d(text='说点什么...').exists(timeout=3)):
            ...
            print("当前不在直播间，继续等待。。。。。。。。")
    updata_pkl("./shuju/" + service + ".pkl", "进行的任务", "开始直播互动")
    #先判断当前是否在直播间
    for i in range(999999):
        result_back = back_to_live_room(d)
        if (result_back == "2"):
            updata_pkl("./shuju/" + service + ".pkl", "进行的任务", "当前直播已结束")
            beisaier_random(d)
        if(result_back != "1"):
            updata_pkl("./shuju/" + service + ".pkl", "进行的任务", "当前找不到直播间")
            return "2"
        print(f"----------开始滑动第{i+1}次----------------------")
        result_yewu = yewu_aaa(d)
        if(result_yewu == "1"):
            update_pkl_add_one("./shuju/" + str(service) + ".pkl", "tongji")
        result_back = back_to_live_room(d)
        if (result_back == "2"):
            updata_pkl("./shuju/" + service + ".pkl", "进行的任务", "当前直播已结束")
            beisaier_random(d)
...
def yewu_aaa(d):
    ...
    if (d(resourceId='com.ss.android.ugc.aweme:id/user_name').exists(timeout=3)):
        random_click_view(d,d(resourceId='com.ss.android.ugc.aweme:id/user_name'))
        time.sleep(5)
    else:
        print("当前直播间没有头像啊，退出。。。。。。。。")
        return

    if (d(descriptionContains='的头像').exists(timeout=3)):
        random_click_view(d, d(descriptionContains='的头像'))
        time.sleep(random.randint(2, 4))
    else:
        print("当前没有头像啊，。。。。。。。。")
        return

    if (d(text='发私信').exists(timeout=3)):
        random_click_view(d,d(text='发私信'))
        time.sleep(5)
    elif (d(description='私信').exists(timeout=3)):
        random_click_view(d,d(description='私信'))
        time.sleep(5)
    else:
        print("当前直播间没有sixin啊，退出。。。。。。。。")
        return


    if (d(description='更多面板').exists(timeout=3)):
        random_click_view(d, d(description='更多面板'))
        time.sleep(random.randint(2, 4))
    else:
        print("当前没有头像啊，。。。。。。。。")
        return

    if (d(descriptionContains='照片').exists(timeout=3)):
        # random_click_view(d, d(description='更多面板'))
        # time.sleep(random.randint(3, 10))
        y = d(descriptionContains='照片').info["bounds"]["top"]
        d.click(100,y+50)
        time.sleep(5)
    else:
        print("当前没有照片啊，。。。。。。。。")
        return
    x_w,y_w = d.window_size()
    if (d(text='编辑').exists(timeout=3)):
        y = d(text='编辑').info["bounds"]["top"]+50
        x = x_w - 280
        print("----------------888----------------")
        print(x,y)
        d.click(x,y)
...
def liveRoom(d,file_path):
    ...
    for i in range(3):
        if (d(text='说点什么...').exists(timeout=3) or d(descriptionContains='表情入口').exists(timeout=3) or d(description='关闭').exists(timeout=3)):
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button1 = QCheckBox("抖音主播私信互动")
    ...
    def init_douyin_tab(self):
        ...
        self.douyin_guanjianzi_wenben = QLabel('直播间主播互动时间间隔:')
