...
def random_long_click_view(d,view):
    bottom = view.info["bounds"]["top"]
    left = view.info["bounds"]["left"]
    ...
    d.long_click(random_x,random_y,1.5)
def shell_neibu(cmd):
    os.system(cmd)
...
def main_control(serial):
    ...
    if(str(task).count("douyinyanghao")):
        ...
        print("")
        main_douyin(serial, d,"douyinyanghao")

    if (str(task).count("shanchusixin")):
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "抖音业务")
        print("")
        main_douyin(serial, d, "shanchusixin")
...
def main_douyin(serial,d,renwu):
    ...
    if (renwu == "douyinyanghao" ):
        # d.app_start("com.ss.android.ugc.aweme")
        # backToHome(d)
        # d.press("back")
        # time.sleep(2)
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "点赞任务中")
        try:
            ...
            print(error_info)
        #backToHome(d)
    if (renwu == "shanchusixin" ):
        # d.app_start("com.ss.android.ugc.aweme")
        # backToHome(d)
        # d.press("back")
        # time.sleep(2)
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "删除私信任务中")
        try:
            shanchusixin(serial,d)
        except Exception as e:
            print(f"发生崩溃了: {e}")
            error_info = traceback.format_exc()
            print("完整错误信息:")
...
def shanchusixin(serial,d):
    ...
    #     time.sleep(0.2)
    tuichu_count = 0
    no_title = ["新关注我的", "互动消息", "陌生人消息","消息"]
    if (str(current_activity).count("com.ss.android.ugc.aweme.lite") > 0):
        while (True):
            bbb = d(resourceId='com.ss.android.ugc.aweme.lite:id/tv_title')
            if (bbb):
                if (bbb):
                    flag = False
                    for i in range(0, len(bbb) - 1):
                        title = bbb[i].get_text()
                        if (title not in no_title):
                            print("title=", title)
                            random_long_click_view(d, bbb[i])
                            time.sleep(1)

                            if (d(text='删除').exists(timeout=3)):  # 删除并清空聊天记录
                                print("有删除")
                                random_click_view(d, d(text='删除'))
                                time.sleep(random.randint(1, 3))
                            elif (d(text='删除会话').exists(timeout=3)):  # 删除并清空聊天记录
                                print("删除会话")
                                random_click_view(d, d(text='删除会话'))
                                time.sleep(random.randint(1, 3))
                            else:
                                print("当前没有删除啊。。。。。。。。")
                                continue
                            if (d(text='仅删除').exists(timeout=1)):  # 删除并清空聊天记录
                                random_click_view(d, d(text='仅删除'))
                                time.sleep(random.randint(1, 3))

                            if (d(text='删除并清空聊天记录').exists(timeout=3)):  # 删除并清空聊天记录
                                random_click_view(d, d(text='删除并清空聊天记录'))
                                time.sleep(random.randint(1, 3))
                                break
                            elif (d(text='仅删除').exists(timeout=1)):  # 删除并清空聊天记录
                                random_click_view(d, d(text='仅删除'))
                                time.sleep(random.randint(1, 3))
                                break
                            elif (d(text='确认删除').exists(timeout=1)):  # 删除并清空聊天记录
                                random_click_view(d, d(text='确认删除'))
                                time.sleep(random.randint(1, 3))
                                break
                            elif (d(textContains='删除').exists(timeout=1)):  # 删除并清空聊天记录
                                random_click_view(d, d(textContains='删除'))
                                time.sleep(random.randint(1, 3))
                                break
                            else:
                                print("当前没有删除并清空聊天记录啊。。。。。。。。")
                                flag = True

            if (d(text='暂时没有更多了').exists(timeout=1)):  # 删除并清空聊天记录

                tuichu_count += 1
                print("退出", tuichu_count)
                if (tuichu_count > 9):
                    print("暂时没有更多了，退出")
                    updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "删除完成")
                    return "88"
    elif (str(current_activity).count("com.ss.android.ugc.aweme") > 0):
        while(True):
            bbb = d(resourceId='com.ss.android.ugc.aweme:id/tv_title')
            if(bbb):
                flag = False
                for i in range(0,len(bbb)-1):
                    title = bbb[i].get_text()
...
def detail(serial,d,from_t="meng"):
    ...
    swipe_count_temp = int(random.uniform(meicituijianhuadongcishu_xiao, meicituijianhuadongcishu_da))
    ...
    xiuxiduochangshijian = int(get_value_by_key_pkl("shuju_config.pkl", "xiuxiduochangshijian"))  # tongchengguanjianzi
    tongchengguanjianzi = get_value_by_key_pkl("shuju_config.pkl", "tongchengguanjianzi")  #
    shouyedianzangailv = int(get_value_by_key_pkl("shuju_config.pkl", "shoucang_gailv"))
    tongchengguanjianzis = str(tongchengguanjianzi).split("_")
    zhengque_id = None
    while (b_count < 100000000):
        print("-----------------------重新开始---------------------------")
        backTo_dy_detail(d)
        beisaier_big(d)
        #time.sleep(1)
        ...
        if(b_count >= int(meihuadongcishu)):
            print("开始休息")
            b_count = 1
            updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "休息中")
            time.sleep(int(xiuxiduochangshijian) * 60)
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "开始工作")
        try:
            flagggg = False
            current_activity = d.app_current()
            print("当前Activity:", current_activity)
            ...
            #
            #     print("jisu----------------")
            #     if (d(resourceId="com.ss.android.ugc.aweme.lite:id/p8i").exists(timeout=1.5)):
            #         print("------you a")
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme.lite:id/p8i").get_text()
            #         print("")
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             time.sleep(0.2)
            #             if ((str(tongchengguanjianzi_temp).count(title_temp) > 0) or (str(title_temp).count(tongchengguanjianzi_temp) > 0)):
            #                 flagggg = True
            #                 break
            #     elif(d(resourceId="com.ss.android.ugc.aweme.lite:id/p8b").exists(timeout=1)):
            #         print("------you a111")
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme.lite:id/p8b").get_text()
            ...
            #                 flagggg = True
            #                 break
            #     elif (d(resourceId="com.ss.android.ugc.aweme.lite:id/p96").exists(timeout=1)):
            #         print("------you a111")
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme.lite:id/p96").get_text()
            #         print("title_temp=", title_temp)
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             time.sleep(0.2)
            #             if ((str(tongchengguanjianzi_temp).count(title_temp) > 0) or (str(title_temp).count(tongchengguanjianzi_temp) > 0)):
            #                 flagggg = True
            #                 break
            #     elif (d(resourceId="com.ss.android.ugc.aweme.lite:id/p=a").exists(timeout=1)):
            #         print("------you a111")
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme.lite:id/p=a").get_text()
            #         print("title_temp=", title_temp)
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             time.sleep(0.2)
            #             if ((str(tongchengguanjianzi_temp).count(title_temp) > 0) or (str(title_temp).count(tongchengguanjianzi_temp) > 0)):
            ...
            #         continue
            # else:
            #     if (d(resourceId="com.ss.android.ugc.aweme:id/2ln").exists(timeout=1.5)):
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme:id/2ln").get_text()
            #         print("title_temp=", title_temp)
            #         print("tongchengguanjianzis=", tongchengguanjianzis)
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             print("tongchengguanjianzi_temp=", tongchengguanjianzi_temp)
            #             if ((str(tongchengguanjianzi_temp).count(title_temp) > 0) or (str(title_temp).count(tongchengguanjianzi_temp) > 0)):
            #                 flagggg = True
            #                 break
            #     elif(d(resourceId="com.ss.android.ugc.aweme:id/2jc").exists(timeout=1)):
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme:id/2jc").get_text()
            #         print("title_temp=", title_temp)
            #         print("tongchengguanjianzis=", tongchengguanjianzis)
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             print("tongchengguanjianzi_temp=", tongchengguanjianzi_temp)
            ...
            #                 flagggg = True
            #                 break
            #     elif (d(resourceId="com.ss.android.ugc.aweme:id/2m9").exists(timeout=1)):
            #         title_temp = d(resourceId="com.ss.android.ugc.aweme:id/2m9").get_text()
            #         print("title_temp=", title_temp)
            #         print("tongchengguanjianzis=", tongchengguanjianzis)
            #         for tongchengguanjianzi_temp in tongchengguanjianzis:
            #             print("tongchengguanjianzi_temp=", tongchengguanjianzi_temp)
            ...
            if (str(current_activity).count("com.ss.android.ugc.aweme.lite") > 0):#textStartsWith

                print("jisu----------------")
                if (zhengque_id != "hhhhhhh"):
                    if (d(textStartsWith='@').exists(timeout=3)):
                        print()
                        temp_y = d(textStartsWith='@').info["bounds"]["top"] - 70
                        title_temp_id = find_id_from_area(d, 180, temp_y)
                        if (title_temp_id != None):
                            title_temp = d(resourceId=title_temp_id).get_text()
                            if (title_temp != None):
                                print("title_temp=", title_temp)
                                print("tongchengguanjianzis=", tongchengguanjianzis)
                                for tongchengguanjianzi_temp in tongchengguanjianzis:
                                    print("tongchengguanjianzi_temp=", tongchengguanjianzi_temp)
                                    if ((str(tongchengguanjianzi_temp).count(title_temp) > 0) or (
                                            str(title_temp).count(tongchengguanjianzi_temp) > 0)):
                                        zhengque_id = title_temp_id
                                        flagggg = True
                                        break
                            else:
                                continue
                    ...
                    print("已经找到队的ID可=", zhengque_id)
                    if (d(resourceId=zhengque_id).exists(timeout=3)):
                        title_temp = d(resourceId=zhengque_id).get_text()
                        print("title_temp=", title_temp)
                        if (title_temp != None):
                            print("title_temp=", title_temp)
                            print("tongchengguanjianzis=", tongchengguanjianzis)
            ...
            if(shouyedianzangailv > random_int):
            ...
            if (d(descriptionContains='缩小评论区').exists(timeout=2)):
                print("放大评论区")
                d(descriptionContains='缩小评论区').click()
                time.sleep(random.randint(1, 4))
            elif(d(descriptionContains='放大评论区').exists(timeout=1)):
                d(descriptionContains='放大评论区').click()
                time.sleep(random.randint(1, 4))

            first_home_filter = get_value_by_key_pkl("shuju_config.pkl", "first_home_filter")
            second_home_filter = get_value_by_key_pkl("shuju_config.pkl", "second_home_filter")
            shijianguanjianzis = []
            if (first_home_filter == "一小时内"):
                shijianguanjianzis.append("1小时前")
            elif (first_home_filter == "三小时内"):
                shijianguanjianzis.append("2小时前")
            elif (first_home_filter == "五小时内"):
                shijianguanjianzis.append("4小时前")
            elif (first_home_filter == "12小时内"):
                shijianguanjianzis.append("11小时前")
            elif (first_home_filter == "一天内"):
                shijianguanjianzis.append("23小时前")
            else:
                shijianguanjianzis.append("23小时前")
            print("shijianguanjianzis=",shijianguanjianzis)
            flaggggggggggggg = False
            huifus = d(descriptionContains="回复 按钮")
            for huifu in huifus:
                print(huifu.info)
                print("----",len(huifu.child()[0].child()))
                if(len(huifu.child()[0].child())<=4):
                    break
                print(huifu.child()[0].child()[4].get_text())
                shijian = huifu.child()[0].child()[4].get_text()
                shijianguanjianzi = shijianguanjianzis[0]

                #for shijianguanjianzi in shijianguanjianzis:
                print("shijianguanjianzi=",shijianguanjianzi)
                if((str(shijian).count("刚刚")>0) or (str(shijian).count("分钟")>0) ):
                    flaggggggggggggg = True
                    print("shouye ok", shijianguanjianzi)
                    break
                elif(str(shijian).count("小时")>0):
                    time_1 = str(shijian)[0:-3]
                    time_2 = str(shijianguanjianzi)[0:-3]
                    print("----------")
                    print(time_1,time_2)

                    if(not time_1.isdigit()):
                        continue
                    if (not time_2.isdigit()):
                        continue
                    print(time_1>time_2)
                    print(int(time_1)>int(time_2))
                    if(int(time_1)>int(time_2)):
                        ...
                        print("shouye ok", shijianguanjianzi)
                        break
                elif (str(shijian).count("昨天") > 0):
                    zuotian_xiaoshi = str(shijian)[2:].split(":")[0]
                    print("zuotian_xiaoshi = ",zuotian_xiaoshi)
                    if (not zuotian_xiaoshi.isdigit()):
                        print("1")
                        continue
                    print("shijianguanjianzi=",shijianguanjianzi)
                    time_2 = str(shijianguanjianzi)[0:-3]
                    ...
                    dangqianxiaoshi = int(current_hour)+24
                    zuotian_xiaoshi  = int(zuotian_xiaoshi)
                    shijiancha = dangqianxiaoshi - zuotian_xiaoshi
                    print(dangqianxiaoshi,zuotian_xiaoshi,shijiancha)
                    if (int(shijiancha) > int(time_2)):
                        print("大于")
                        continue
                    else:
                        flaggggggggggggg = True
                        print("shouye ok,带有昨天的满足", shijianguanjianzi)
                        break
            ...
            shijianguanjianzis.clear()
            if (flaggggggggggggg == False):
                print("shouye bu ok")
                continue
            if (second_home_filter == "一小时内"):
                shijianguanjianzis.append("1小时前")
                # shijianguanjianzis.append("刚刚")
            elif (second_home_filter == "三小时内"):
                shijianguanjianzis.append("2小时前")
            elif (second_home_filter == "五小时内"):
                shijianguanjianzis.append("4小时前")
                # shijianguanjianzis.append("分钟前")
            elif (second_home_filter == "12小时内"):
                shijianguanjianzis.append("11小时前")
                # shijianguanjianzis.append("分钟前")
            elif (second_home_filter == "一天内"):
                shijianguanjianzis.append("23小时前")
            else:
                shijianguanjianzis.append("23小时前")
            clicked = []
            for i in range(swipe_count_temp):
                print("i=",i)
                ...
                for huifu in huifus:
                    ...
                    if ((str(shijian).count("刚刚") > 0) or (str(shijian).count("分钟") > 0)):
                        flaggggggggggggg = True
                        print("shouye ok", shijianguanjianzi)
                        if(huifu.info["contentDescription"] not in clicked):
                            b_count += 1
                            huifu.child(descriptionContains="未选中").click()
                            clicked.append(huifu.info["contentDescription"])
                            update_pkl_add_one("./shuju/" + str(serial) + ".pkl", "tongji")
                    elif (str(shijian).count("小时") > 0):
                        time_1 = str(shijian)[0:-3]
                        time_2 = str(shijianguanjianzi)[0:-3]
                        ...
                        current_hour = current_time.hour
                        print(f"当前小时: {current_hour}")
                        if ((int(current_hour) + 24 - int(zuotian_xiaoshi) > int(time_2))):
                            continue
                        else:
                            flaggggggggggggg = True
                            print("shouye ok,带有昨天的满足", shijianguanjianzi)
                ...
                if (d(text="暂无评论").exists(timeout=1)):
                    break
                if (d(text="期待你的评论").exists(timeout=1)):
                    break
                if (d(text="暂时没有更多了").exists(timeout=1)):
                    break
                beisaier_random(d)
                time.sleep(1)
...
def backTo_dy_detail(d):
    ...
    while (dd < 3):
        if (d(textStartsWith="@").exists(timeout=3)):
            if (d(textStartsWith="首页").exists(timeout=3)):
                return "1"
        time.sleep(1)
        d.press("back")
        time.sleep(1)
...
def beisaier_random(d,Diract="up"):
    ...
    # if Diract == "up":
    #     random_start_point_x = random.uniform(0.3, 0.6)
    #     random_start_point_y = random.uniform(0.6, 0.63)
    #     random_end_point_x = random.uniform(0.2, 0.99)
    #     random_end_point_y = 0.01
    #
    #     start_point = (width * random_start_point_x, height * random_start_point_y)  # 屏幕中下位置
    ...
    #
    # # 执行贝塞尔曲线滑动
    # random_num = random.uniform(0.3, 0.3)
    # swipe_along_bezier(d, start_point, end_point, control_points, steps=15, duration=random_num)
    d.swipe(100,1000,100,1,0.2)

    # 等待一下
    time.sleep(1)
def beisaier_big(d,Diract="up"):
    # 获取屏幕尺寸
    width, height = d.window_size()

    # 设置起点和终点
    if Diract == "up":
        random_start_point_x = random.uniform(0.3, 0.3)
        random_start_point_y = random.uniform(0.78, 0.8)
        random_end_point_x = random.uniform(0.3, 0.3)
        random_end_point_y = 0.01
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button1 = QCheckBox("抖音点赞")
        self.radio_button1.setChecked(True)
        self.radio_button2 = QCheckBox("删除私信")
    ...
    def init_douyin_tab(self):
        ...
        self.radio_button_dy_task0 = QLabel("评论首页时间过滤:")
        ...
        self.first_home_filter = QComboBox()
        self.first_home_filter.addItems(["一小时内", "三小时内", "五小时内", "12小时内", "一天内"])
        self.second_home_filter = QComboBox()

        self.radio_button_dy_task123 = QLabel("评论点赞时间过滤:")

        self.second_home_filter.addItems(["一小时内", "三小时内", "五小时内", "12小时内", "一天内"])
        self.horizontal_layout_dy_task.addWidget(self.first_home_filter)
        self.horizontal_layout_dy_task.addWidget(self.radio_button_dy_task123)
        self.horizontal_layout_dy_task.addWidget(self.second_home_filter)
        ...
        #抖音视频推荐单次滑动次数
        self.douyin_guanjianzi_wenben_huadongcishu = QLabel('评论区上滑次数:')
        ...
        self.douyin_shoucang_gailv = QLabel('首页点赞概率')
        self.douyin_shoucang_gailv.setFixedWidth(80)
        ...
        self.douyin_meihuadong_gailv = QLabel('每点赞')
    ...
    def save_config(self):
        try:
            ...
            updata_pkl_config_mianban("dianzan_gailv", self.dianzan_gailv.text())#dianzan_gailv
            ...
            updata_pkl_config_mianban("first_home_filter", self.first_home_filter.currentText())
            updata_pkl_config_mianban("second_home_filter", self.second_home_filter.currentText())
