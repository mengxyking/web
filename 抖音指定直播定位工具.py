...
def operate_device(serial, curr_zhubo_nicheng):
    # result_j = judge()
    # if (result_j == False):
    #     print("当前需要联系")
    #     return
    ...
    while (True):
        try:
            print("while------------")
            result = duozhubo(serial, curr_zhubo_nicheng)
            if (result == "66"):
                print("运行结束")
            ...
            print(serial, "--------", "完整错误信息:")
            print(error_info)
            operate_device(serial,curr_zhubo_nicheng)
...
def duozhubo(serial,curr_zhubo_nicheng):
    # if (len(fudai) > 0):
    ...
    while (True):
        ...
        print(serial,"--------","应该进入的直播间", curr_zhubo_nicheng)
        try:
            result_main = main(serial,curr_zhubo_nicheng)
            if(result_main == "66"):
                return "66"
        except:
            error_info = traceback.format_exc()
...
def main(serial,curr_zhubo_nicheng):
    ...
    updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "进入直播间")#fudai_guanjianzi
    xianzhi_cishu = get_value_by_key_pkl("shuju_config.pkl", "fudai_guanjianzi")
    print(serial + "---------", xianzhi_cishu)
    if ("fudai" in "fudai"):
        time.sleep(1)
        ...
        if ("通过推荐进入" == "通过推荐进入"):
            for i in range(100000):
                ...
                if(i > int(xianzhi_cishu) ):
                    #d.app_stop(package_name="com.ss.android.ugc.aweme")
                    return "66"
                ...
                #print("当前在循环里头")
                try:
                    if (d(text=f"@{curr_zhubo_nicheng}").exists(timeout=2)):
                        # beisaier_random(d)
                        time.sleep(2)
                        #continue
                        if (d(text="点击进入直播间").exists(timeout=6)):
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"抖音指定直播定位工具，欢迎:{get_real_device_id()}")
        ...
        self.combo_box.addItems(["通过推荐进入"])

        self.radio_button4.setChecked(True)
        self.radio_button6 = QCheckBox("指定直播定位")
        ...
        file_temp_path_comment = get_value_by_key_pkl("shuju_config.pkl", "file_path_comment1")
        ...
        self.clear_task_config_button = QPushButton('打开直播间', self)
        self.clear_task_config_button.clicked.connect(self.clear_task)

        self.open_douyin = QPushButton('打开抖音', self)
        self.open_douyin.clicked.connect(self.open_dy)
        ...
        self.label_from111_fudai_guanjianzi = QLabel('      滑动次数限制:')
        ...
        self.label_from111_fudai_guanjianzi_dengdaishijian = QLabel('  ')
        self.label_from111_fudai_guanjianzi_dengdaishijian.setFixedWidth(250)
        ...
        self.fudai_guanjianzi_kongge_dengdaishijian = QLabel("")
        ...
        self.label_from_search_shouye_pinglunjiangeshijian = QLabel('              dy直播间链接:')
        self.label_from_search_shouye_pinglunjiangeshijian.setFixedWidth(150)
        ...
        self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao.setFixedWidth(400)
        ...
        self.button_gang.addWidget(self.open_douyin)
        self.execute_button_delete = QPushButton("关闭抖音")
    ...
    def execute_button_clicked(self):
        ...
        for temp in self.selected_ids:
            ...
            updata_pkl("./shuju/" + temp + ".pkl", "进行的任务", "寻找指定直播间")
    ...
    def thread_temp(self, tasks,curr_zhubo_nicheng):
        ...
        for serial in self.selected_ids:
            ...
            thread = threading.Thread(target=operate_device, args=(serial, curr_zhubo_nicheng))
            # 搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
    ...
    def clear_task(self):
        ...
        for serial in self.selected_ids:
            #print("---------------->", get_value_by_key_pkl("config.pkl", serial))
            thread = threading.Thread(target=self.open_dyLiveRoom, args=(serial, self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao.text(),))
            # 搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
            # threads.append(thread)
            thread.start()
            time.sleep(0.1)
    ...
    def open_dy(self):
        ...
        for serial in self.selected_ids:
            #print("---------------->", get_value_by_key_pkl("config.pkl", serial))
            thread = threading.Thread(target=self.open_dyhome, args=(serial,))
            # 搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
            # threads.append(thread)
            thread.start()
            time.sleep(0.01)
    ...
    def open_dyLiveRoom(self,service,url):
        cmd = f'adb -s {service} shell am start -a android.intent.action.VIEW -d "snssdk1128://webview?url={str(url).strip()} " -p com.ss.android.ugc.aweme'
        print(cmd)
        shell_neibu(cmd)
    def open_dyhome(self,service):
        cmd = f'adb -s {service} shell am start -n com.ss.android.ugc.aweme/.main.MainActivity'
        print(cmd)
        shell_neibu(cmd)

    def close_dy(self,service):
        cmd = f'adb -s {service} shell am force-stop  com.ss.android.ugc.aweme'
        print(cmd)
        shell_neibu(cmd)
    def execute_delete_button_clicked(self):
        for serial in self.selected_ids:
            # print("---------------->", get_value_by_key_pkl("config.pkl", serial))
            thread = threading.Thread(target=self.close_dy,args=(serial, ))
            # 搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
            # threads.append(thread)
            thread.start()
            time.sleep(0.1)
