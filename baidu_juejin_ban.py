...
def watch(d):
    time.sleep(5)
    while(True):
        if(d(text="任务完成").exists(timeout=0.2)):
            print("有任务完成，点击")
            x1,y1,x2,y2 = d(text="任务完成").bounds()
            d.click(x2+253,y1-91)
        else:
            print("meiyou完成")
        if (d(text="任务完成").exists(timeout=0.2)):
            print("有任务完成，点击")
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu):
    ...
    # shell_neibu(f"adb -s {serial} shell am start -n com.baidu.searchbox.lite/com.baidu.searchbox.MainActivity")
    # time.sleep(3)
    # shell_neibu(f"adb -s {serial} shell am start -n com.baidu.searchbox.lite/com.baidu.searchbox.MainActivity")
    # time.sleep(3)
    ...
    #shell_neibu(f"adb -s {serial} shell monkey -p com.baidu.searchbox.lite --throttle 500 100")
    time.sleep(1)
    # 注册一个watcher，当弹窗上的“确定”按钮出现时，点击它
    d.watcher.when("以后再说").click()
    #d.watcher.when("任务完成").press("back")
    # 开始后台监控
    d.watcher.start()
    #d.app_start(package_name="com.baidu.searchbox.lite",stop=True)
    # thread1 = threading.Thread(target=watch, args=(d,))
    # # 启动线程
    # thread1.start()
    zong_count1 = 0
    zongyewu(d)
    ...
    # d.watcher.when(check_element_by_id("com.baidu.searchbox.lite:id/bf2"))
    # time.sleep(3)
    # d.swipe_points([(500, 1200), (600, 608)], 0.2)
    # time.sleep(3)
    # d.swipe_points([(300, 1200), (400, 608)], 0.2)
    # time.sleep(3)
    #
    #
    # inter_count = 0
    # while(inter_count < 10):
    #     if (d(text="去提现")):
    #         print("当前在任务列表了")
    #         break
    #     tabs = d(text="视频").exists(timeout=5)
    #     print("tabs=", tabs)
    #     if (tabs):
    #         print("zhaodaole")
    #         print(d(text="视频").bounds())
    #         x, y, m, n = d(text="视频").bounds()
    #         d.click(int(d.window_size()[0]) / 2-8, y+10)
    #         time.sleep(4)
    #         d.swipe_points([(500, 1200), (600, 800)], 0.2)
    #         time.sleep(2)
    #
    #
    #
    #     # else:
    #     #     if (d(text="视频")):
    #     #         x1, y1, x2, y2 = d(text="视频").bounds()
    #     #         d.click(x1 + 10, y1 + 10)
    #     #         time.sleep(2)
    #     #         d.click(int(d.window_size()[0]) / 2, y1)
    #     #         time.sleep(2)
    #
    #     inter_count+=1
    #
    ...
    # while(yewu_count < 10):
    #     # 开宝箱
    #     if (d(text="开宝箱得金币").exists(timeout=3)):
    #         x1,y1,x2,y2 = d(text="开宝箱得金币").bounds()
    #         d.click(x1+10,y1+10)
    #         d.click(x1 + 10, y1 + 10)
    #         print("已经点完金币了")
    #         time.sleep(3)
    #
    #         if (d(textContains='观看视频').exists(timeout=3)):
    #             print("获取过宝箱了")
    #             d(textContains='观看视频').click()
    #             watch_ad(d)
    #             time.sleep(3)
    #
    #     # 看广告赚钱
    #     if (d(text="看广告赚钱").exists(timeout=3)):
    #         while (True):
    #             x1, y1, x2, y2 = d(text="看广告赚钱").bounds()
    #             print("y2=",y2)
    #             print("d.window_size()[1] - 350 = ",d.window_size()[1] - 350)
    #             if (y2 > d.window_size()[1] - 350):
    #                 print("需要滑动")
    #                 d.swipe_points([(500, 1000), (600, 808)], 0.2)
    #                 time.sleep(1)
    #             else:
    #                 break
    #     if(d(text = "随心搜").exists(timeout=3)):
    #         d(text="随心搜").click()
    #         time.sleep(20)
    #         d.press("back")
    #         time.sleep(3)
    ...
    #     if (d(text="看广告赚钱").exists(timeout=3)):
    #         x1, y1, x2, y2 = d(text="看广告赚钱").bounds()
    #         d.click(d.window_size()[0] - 80, y2 + 10)
    #
    #         result = watch_ad(d)
    #         print("result=", result)
...
def zongyewu(d):
    try:
        zong_count1 = 0
        while (True):
            if (d(text="广告").exists(timeout=0.2)):
                bb = 0
                while(bb<10):
                    print("广告页面")
                    watch_ad(d)
                    time.sleep(8)
                    d.click(538, int(math.ceil(0.58*d.info["displayHeight"])))
                    bb+=1
            if (d(resourceId="com.baidu.searchbox.lite:id/bt1").exists(timeout=0.2)):
                print("视频页面")
                #watch_video(d)
            zong_count1 += 1
            time.sleep(1)
...
def watch_video(d):
    while(True):
        if (d(resourceId="com.baidu.searchbox.lite:id/bt1").exists(timeout=2)):
            d.swipe_points([(500, 1200), (600, 800)], 0.2)
            time.sleep(2)
        else:
            print("当前不在视频页面了")
...
def watch_ad(d):
    count = 0
    while(count<25):
        if(d(textContains="已领取").exists(timeout=1)):
            d(resourceId="com.baidu.searchbox.lite:id/iap").click()
            time.sleep(3)
            if(d(text="残忍离开").exists(timeout=2)):
                d(text="残忍离开").click()
                time.sleep(2)
                # if (d(textContains="看广告再得").exists(timeout=5)):
                #     d(textContains="看广告再得").click()
            return 1
        else:
            time.sleep(3)
        ...
        if(count>23):
            ...
            while(temp_count<10):
                d.press("back")
                if (d(text="残忍离开").exists(timeout=3)):
                    d(text="残忍离开").click()
                    return 1
                temp_count+=0
            break
    ...
    if(d(resourceId="com.baidu.searchbox.lite:id/iap")):
        d(resourceId="com.baidu.searchbox.lite:id/iap").click()
    return 0
...
def backToHome(d):
    ...
    time.sleep(3)
    while(dd < 10):
        elements = d(text='Home')  # 获取所有文本为'some_text'的元素
        #print(len(elements))
        if(len(elements)>0):
            return "1"
        elements = d(text='主页')  # 获取所有文本为'some_text'的元素
        # print(len(elements))
        if (len(elements) > 0):
            return "1"
        time.sleep(1.5)
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.radio_button1 = QCheckBox("百度极速")
        self.radio_button1.setChecked(True)
        self.radio_button2 = QCheckBox("抖音极速")
        self.radio_button2.setChecked(True)
        self.radio_button3 = QCheckBox("快手极速")
    ...
    def execute_button_clicked(self):
        ...
        for temp in self.selected_ids:
            ...
            updata_pkl("./shuju/" + temp + ".pkl", "进行的任务", "百度极速")
