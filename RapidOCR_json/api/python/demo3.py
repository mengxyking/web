...
class OCRProcessor:
    def __init__(self,id: int):
        ...
        self.id = id  # 标记对象ID，方便调试
        self.busy = False  # 标记是否正在处理任务
    ...
    def getQianmianAndTexts_by_data_from_small_area_zhineng(self, det, txt):
        # 创建一个字典用于存储文本和对应的中心坐标（保留原有注释，如需可删除）
        text_coordinates = {}
    ...
    def getCount_by_data_from_small_area(self, det, txt, x1, x2, y1, y2):
        # 创建一个字典用于存储文本和对应的中心坐标
        count = 0
        if txt == "22":
            return None
    ...
    def getCount_BY_PaddleOCRJsons_area_from_alldata(self, alldata, txt, x1, x2, y1, y2):
        # getObj = self.ocr.run(image)
        # print(f'图片识别完毕，状态码：{getObj["code"]} 结果：\n{getObj["data"]}\n')
        # print(getObj["data"])
        # center_coordinatess = []
    ...
    #         print(f"📌 OCR{self.id} 处理：{imgPath}")
    #         bb = self.getAllData_test(imgPath)
    #         #print(bb)
    #         time_xiaohao = time.time() - start_time
    #         print(f"第{1}次耗时=", time_xiaohao)
    #         if(time_xiaohao < 1):
    #             print("当前图片识别失败了啊啊啊啊啊啊啊啊啊啊啊啊啊")
    #             return "22"
    #         return bb
    ...
    def yewu(self, imgPath):
        # 1. 前置校验：空值/无效标识直接返回
        if imgPath is None or imgPath == "1":
            return "1"

        # 2. 校验图片路径有效性
        if not isinstance(imgPath, str) or not os.path.exists(imgPath):
            print(f"❌ OCR{self.id} 图片路径无效：{imgPath}")
            return "22"  # 统一返回识别失败标识

        self.busy = True
        retry_count = 2  # 重试次数，可根据需求调整
        final_result = None

        try:
            # 3. 增加重试机制，应对偶发的OCR识别异常
            for attempt in range(retry_count + 1):
                start_time = time.time()
                print(f"📌 OCR{self.id} 第{attempt + 1}次处理：{imgPath}")

                try:
                    # 核心OCR识别逻辑
                    final_result = self.getAllData_test(imgPath)
                    time_xiaohao = time.time() - start_time
                    print(f"✅ OCR{self.id} 第{attempt + 1}次耗时={time_xiaohao:.2f}秒")

                    # 4. 合理的识别结果校验（替代原有的"耗时<1秒"判定）
                    if final_result is not None and final_result != "":
                        break  # 识别成功，退出重试
                    elif attempt < retry_count:
                        print(f"⚠️ OCR{self.id} 第{attempt + 1}次识别结果为空，准备重试...")
                        time.sleep(0.5)  # 重试前短暂等待，避免高频调用

                except Exception as inner_e:
                    # 单次重试失败，打印日志但不终止
                    print(f"❌ OCR{self.id} 第{attempt + 1}次处理异常：{inner_e}")
                    if attempt == retry_count:  # 最后一次重试仍失败
                        raise  # 抛出异常，进入外层异常处理

            # 5. 最终结果校验：仍无有效结果则返回失败标识
            if final_result is None or final_result == "":
                print(f"❌ OCR{self.id} 多次重试后仍识别失败：{imgPath}")
                return "22"
            ...
            # 6. 全局异常捕获：记录详细错误信息
            print(f"💥 OCR{self.id} 处理最终异常：{e}")
            error_info = traceback.format_exc()
            print(error_info)
            return "22"  # 异常时统一返回失败标识，而非None，避免下游处理异常

        finally:
            # 7. 确保busy状态重置（无论成功/失败）
            self.busy = False
...
def get_files_in_dir(dir_path: str, return_relative_path: bool = False) -> list:
    ...
    :param return_relative_path: 是否返回相对路径（False返回文件名，True返回相对路径）
    :return: 文件列表
    """
    # 校验目录是否存在
    ...
    for item in os.listdir(dir_path):
        ...
        if os.path.isfile(item_path):
            if return_relative_path:
                file_list.append(item_path)  # 返回相对路径
            else:
                file_list.append(item)  # 仅返回文件名
    return file_list

#
# testImgPath = r"E:\360MoveData\Users\Administrator\Desktop\Screenshot_20260302_194010_com_ss_android_ugc_awe.jpg"
# script_dir = os.path.dirname(os.path.abspath(__file__))
# # 拼接图片的绝对路径
...
# ocr = OCRProcessor()  # 全局实例（加锁后串行调用）
# alldata = ocr.yewu(testImgPath)
# print(alldata)
#
# def tupianchuli(i):
#     print(f"当前是第{i}次")
#     with global_lock:  # 锁覆盖全流程，确保串行
#         # 2. 生成唯一的黑白图片路径，避免IO冲突
#         alldata = ocr.yewu(i)
#         print(alldata)
...
#     print(alldata)
#
# lists = get_files_in_dir(r"C:\Users\Administrator\Desktop\111111")
# for i in lists:
#     threading.Thread(target=tupianchuli,args=(r"C:\Users\Administrator\Desktop\111111"+"/"+i,)).start()
#tupianchuli(1)

# print(ocr.getPoint_qiandao(alldata,30,300,100,400))

#vvv = ocr.getPoint_qiandao(alldata,30,300,100,400)
# for temp in vvv:
#     if(str(temp).count("金币")):
#         lists = str(temp).split("金币")
#         if(str(lists[0]).isdigit()):
...
#print(ocr.getQianmianAndTexts_by_data_from_small_area_zhineng(alldata,"金币"))
