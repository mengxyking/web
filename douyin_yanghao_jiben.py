...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin):
    ...
    while(True):
        try:
            import datetime
            import time
            if(int(datetime.datetime.now().timestamp()) > 1769689322):
                return
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu):
    ...
    if("yanghao" in task):
        ...
        while (count_swipe < swipe_count):
            ...
            x11,y11 = d.window_size()
            d.swipe_points([(500, int(y11)-250), (600, 200)], 0.2)
            ...
            if (d(descriptionContains='收藏').exists(timeout=3)):
                ...
                #     print("当前可以pinglun")
                #     result = comment(d, "1", serial, comment_path)
                #     if(result != "1"):
                #         return
                #     time.sleep(1.5)
    ...
    #     count_xiao = 0
    #     while(count_xiao<6):
    #         d.swipe_points([(100, 1000), (700, 1000)], 0.2)
    #         time.sleep(1)
    #
    #         if(d(text="商城").exists(timeout=1)):
    #             d(text="商城").click()
    #             break
    #         count_xiao+=1
    #     time.sleep(2)
    #     if (d(textContains="搜索").exists(timeout=1)):
    ...
    #     if ((len(gouwu) > 1) and (gouwu != None)):
    #         print("搜索词符合规范")
    #     else:
    #         print("搜索词为空")
    #         return
    #     if(str(gouwu).count("-")>0):
    #         shell_neibu(f"adb -s {serial} shell  am broadcast -a clipper.set -e text " + str(gouwu).split("-")[0])
    #         shell_neibu(f"adb -s {serial} shell input  keyevent 279")
    #     time.sleep(2)
    #     if (d(textContains="搜索").exists(timeout=1)):
    #         d(textContains="搜索").click()
    ...
    #         return
    #     time.sleep(5)
    #     if (d(textContains="抖音旗舰").exists(timeout=1)):
    #         d(textContains="抖音旗舰").click()
    #     else:
    #         return
    #     time.sleep(5)
    #
    #     dianpuming = str(str(gouwu).split("-")[1].strip())
    #     print("店铺名-"+dianpuming)
    #     flag = False
    #     while(True):
    #
    #         temp = d(className = "com.lynx.tasm.behavior.ui.LynxFlattenUI")
    #         for bb in temp:
    #             if(compare(dianpuming,bb.get_text())):
    #                 bb.click()
    #                 time.sleep(2)
    #                 flag = True
    #                 break
    ...
    #             break
    #         time.sleep(1)
    #         d.swipe_points([(500, 1200), (500, 1000)], 0.2)
    #         time.sleep(1)
    ...
    #     time.sleep(1)
    #     small_count = 0
    #     while(small_count < 10):
    #         if (d(text="支付宝").exists(timeout=1)):
    #             d(text="支付宝").click()
    #             time.sleep(3)
    #             break
    #         if (d(text="更多支付方式").exists(timeout=1)):
    #             d(text="更多支付方式").click()
    #             time.sleep(1)
    #         time.sleep(1)
    #         d.swipe_points([(500, 1200), (500, 1000)], 0.2)
    ...
    #         small_count+=1
    #
    #     time.sleep(int(random.randint(30, 300)))
    #
    #     if (d(text="立即支付").exists(timeout=30)):
    #         d(text="立即支付").click()
    #         time.sleep(3)
    #     else:
    #         return
    #     #print(d(text="加入购物车").bounds())
    #     time.sleep(10)
    #     if (d(text="查看全部").exists(timeout=1)):
    #         d(text="查看全部").click()
    #         time.sleep(3)
    #     #else:
    #         #return
    #
    #     time.sleep(3)
    #     if (d(text="找朋友帮忙付").exists(timeout=1)):
    #         d(text="找朋友帮忙付").click()
    #         time.sleep(3)
    #     else:
    #         return
    #
    #     time.sleep(3)
    #     if (d(text="输入手机号、邮箱、好友姓名查找").exists(timeout=1)):
    #         d(text="输入手机号、邮箱、好友姓名查找").click()
    #         time.sleep(3)
    #     else:
    #         return
    #
...
def comment(d,language,serial,comment_path):
    ...
    if (d(className='android.widget.EditText').exists(timeout=3)):
        print("点击评论")
        d(className='android.widget.EditText').set_text(comment_t)
        time.sleep(1.5)
    elif(d(text="作者仅允许自己评论").exists(timeout=3)):
        return "66"
    else:
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"dy养号脚本，欢迎:{get_real_device_id()}")
        ...
        file_temp_path_comment = get_value_by_key_pkl("shuju_config.pkl", "file_path_comment222")
    ...
    def showDialog_comment(self):
        ...
        if dialog.exec() == QFileDialog.DialogCode.Accepted:
            ...
            if selected_files:
                ...
                updata_pkl_config_mianban("file_path_comment222", selected_file)
                #self.excel_file = selected_file
                #self.import_config()
                #self.refresh_pkl_files_test()
