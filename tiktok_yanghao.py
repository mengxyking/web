...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu):
    ...
    import datetime
    import time
    if (int(datetime.datetime.now().timestamp()) > 1751855682):
        return
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu):
    ...
    # 开始后台监控 fullId("com.ss.android.ugc.trill:id/noi")
    d.watcher.start()
    if(count_zong == 0):
        d.app_start(package_name="ca.zgrs.clipper")
        time.sleep(1)
        d.app_start(package_name="com.zhiliaoapp.musically", stop=True)
    else:
        d.app_start(package_name="ca.zgrs.clipper")
        time.sleep(1)
        d.app_start(package_name="com.zhiliaoapp.musically")
        backToHome(d)
    ...
    if ((d(text='Home').exists(timeout=3)) or (d(text='首页').exists(timeout=3))):
        print("当前在首页了。。。。。。。。")
    else:
        print("当前bu在首页了。。。。。。。。")
        return
    ...
    if ((d(text='Home').exists(timeout=3)) or (d(text='首页').exists(timeout=3))):
        ...
        if(d(text='Home').exists(timeout=3)):
            language = 0
        else:
            language = 1

        print("当前语言是。。。。。。。。",language)
    ...
    if(language == 1):
        ...
        if (d(text='Videos').exists(timeout=3)):
            d(text='Videos').click()
            time.sleep(3)
        else:
            print("当前没有视频tab啊。。。。。。。。")
