        ...
        try:
            ...
            if (result == "88"):
                ...
                if(shifouguanbidouyin == True):
                    ...
                    cmd_back = f"adb -s {serial} shell input keyevent 4"
                    shell_neibu(cmd_back)
                    time.sleep(0.5)
                    shell_neibu(cmd_back)
                    time.sleep(0.5)
                    shell_neibu(cmd_back)
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai):
    ...
    if ("fensituan" in task):
        ...
        # if (d(resourceId="com.ss.android.ugc.aweme:id/sgb").exists(timeout=8)):
        #     d(resourceId="com.ss.android.ugc.aweme:id/sgb").click()
        #     time.sleep(5)
        #
        # else:
        #     print("当前没有搜索框。。。。。。。。")  #
        ...
        if (d(text="关注").exists(timeout=8)):
            d(text="关注").click()
            time.sleep(5)

        if (d(descriptionContains="粉丝团").exists(timeout=8)):
            d(descriptionContains="粉丝团").click()
            time.sleep(5)
        ...
        if (d(descriptionContains="加入粉丝团").exists(timeout=8)):
            d(descriptionContains="加入粉丝团").click()
            time.sleep(5)
        elif(d(text="我的等级特权").exists(timeout=8)):
            print("....")
        else:
            print("当前没有搜索框。。。。。。。。")  #
            return
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.radio_button3 = QCheckBox("加粉丝团之后是否关闭抖音")
        self.radio_button3.setChecked(True)
        self.radio_button4 = QCheckBox("加粉丝团")
        ...
        self.label_from111_guanzhu = QLabel('账号名称:')
        self.label_from111_guanzhu.setFixedWidth(50)
