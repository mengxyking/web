...
def operate_device(serial):
    ...
    while(True):
        #try:
            import datetime
            import time
            if(int(datetime.datetime.now().timestamp()) > 1842548800005):
                return
...
def main_control(serial):
    ...
    if(str(task).count("douyinyanghao")):
        ...
        print("")
        result_douyin = main_douyin(serial, d)
        if(result_douyin == "99"):
            return "99"
    ...
    #     updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "小红书业务")
    #     print("")
    #
    #     len_douyins = 1
    #     # d.app_start(package_name="com.ss.android.ugc.aweme")
    ...
    #     if (len(douyinicons) >= 1):
    #         print("当前有最少两个小红书", len(douyinicons))
    #
    #         for i in range(len(douyinicons)):
    #             # cmd = f'adb -s {serial} shell am start -n "com.ss.android.ugc.aweme/.splash.SplashActivity"'
    #             cmd = f'adb -s {serial} shell input keyevent 3'
...
def main_douyin(serial,d):
    ...
    if (1 == 1 ):
        ...
        updata_pkl("./shuju/" + serial + ".pkl", "进行的任务", "同城任务中")
        try:
            result_tongcheng = douyin_tongcheng(serial,d)
            if(result_tongcheng == "99"):
                return "99"
        except Exception as e:
            print(f"发生崩溃了: {e}")
            error_info = traceback.format_exc()
...
def detail(serial,d,from_t="meng"):
    ...
    pinglunshuliangxianzhi = int(get_value_by_key_pkl("shuju_config.pkl", "dianzan_gailv"))
    dianzanshuliangxianzhi = int(get_value_by_key_pkl("shuju_config.pkl", "shoucang_gailv"))
    zong_flag = 0
    cc = []
    pinglun_count = 0
    dianzan_count = 0
    ...
    while (b_count < swipe_count_temp):
        ...
        dianzan_gailv_dianzan = int(get_value_by_key_pkl("shuju_config.pkl", "dianzan_gailv_dianzan"))
        ...
        try:
            ...
            if (d(textContains="女").exists(timeout=3)):
                # d(textStartsWith="@").click()
                print("当前是女性")
                time.sleep(1)
                nv_flag = True
                d.press("back")
                count_temp = random.randint(1, 3)
                time.sleep(count_temp)
            if(nv_flag == True):
                print("当前是女性")
                print("")
                ...
                if (d(descriptionContains='放大评论区').exists(timeout=3)):
                    ...
                    print("当前没有放大评论区 。。。。。。。。")
                    #return
                if (random_boolean_with_probability(int(dianzan_gailv_dianzan))):  #
                    print("当前命中点赞")

                    for bb in range(likes):
                        weixuanzhongs = d(descriptionContains='赞')
                        # if(len(weixuanzhongs)<= likes):
                        #     likes = weixuanzhongs
                        print(f"需要点{likes}个赞")
                        for i in range(len(weixuanzhongs)):
                            print("i=",i)
                            #if (d(descriptionContains='赞').exists(timeout=3)):
                            if(str(weixuanzhongs[i].info).count("未选中")):
                                print("点击未选中")
                                weixuanzhongs[i].click()
                                print("未选中")
                                dianzan_count += 1
                                time.sleep(random.randint(1,3))
                                break
                ...
                if(int(pinglun) > 0 ):
                    if (random_boolean_with_probability(int(pinglun))):#
                        if (pinglun_count < pinglunshuliangxianzhi):
                            print("当前可以pinglun")
                            language = 0
                            comment_path = 0
            ...
            if (dianzan_count >= dianzanshuliangxianzhi):
                # return "99"
                zong_flag += 1
                cc.append("dianzan")
            # if (from_t == "sousuo"):
            ...
            #         backTo_dy_detail(d)
            print("--------->",zong_flag,dianzan_count,dianzanshuliangxianzhi,pinglun_count,pinglunshuliangxianzhi)
            print(cc)
            #if(zong_flag > 1):
            if("pinglun" in cc):
                if("dianzan" in cc):
...
def backToHome(d):
    ...
    while(dd < 10):
        ...
        elements111 = d(text='推荐')  # 获取所有文本为'some_text'的元素
        elements222 = d(descriptionContains='评论')  # 获取所有文本为'some_text'的元素
        #print(len(elements))
        if(len(elements)>0):
            return "1"
        if (len(elements111) > 0):
            return "1"
        if (len(elements222) > 0):
            return "1"
...
def backTo_dy_detail(d):
    ...
    time.sleep(1)
    while (dd < 3):
        if (d(textStartsWith="@").exists(timeout=1)):
            if (d(text="我").exists(timeout=1)):
                return "1"
            if (d(text='点击进入直播间').exists(timeout=1)):
                return "1"
        time.sleep(1)
...
def beisaier(d,Diract="up"):
    ...
    if Diract == "up":
        random_start_point_x = random.uniform(0.3, 0.6)
        random_start_point_y = random.uniform(0.79, 0.85)
        random_end_point_x = random.uniform(0.2, 0.99)
        random_end_point_y = 0.01
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button1 = QCheckBox("抖音同城")
    ...
    def init_douyin_tab(self):
        ...
        self.douyin_dianzan_gailv = QLabel('评论数量限制')
        ...
        self.douyin_dianzan_gailv_baifenhao = QLabel('个')
        ...
        self.douyin_shoucang_gailv = QLabel('点赞数量限制')
        ...
        self.douyin_dianzan_gailv_baifenhao1 = QLabel('个')
        ...
        self.douyin_meihuadong_gailv.setFixedWidth(39)
        ...
        # 点赞概率
        self.h_layout_gailv_dianzan = QHBoxLayout()
        self.h_layout_gailv_dianzan.setContentsMargins(1, 0, 0, 0)  # 设置布局边距
        self.h_layout_gailv_dianzan.setSpacing(0)  # 设置控件间距
        self.h_layout_gailv_dianzan.setAlignment(Qt.AlignmentFlag.AlignLeft)

        self.douyin_dianzan_gailv_dianzan = QLabel('点赞概率')
        self.douyin_dianzan_gailv_dianzan.setFixedWidth(50)

        dianzan_gailv_dianzan = get_value_by_key_pkl("shuju_config.pkl", "dianzan_gailv_dianzan")
        if (dianzan_gailv_dianzan != None):
            self.dianzan_gailv_dianzan = QLineEdit(dianzan_gailv_dianzan)
        else:
            self.dianzan_gailv_dianzan = QLineEdit("50")
        self.dianzan_gailv_dianzan.setFixedWidth(50)

        self.douyin_dianzan_gailv_baifenhao_dianzan = QLabel('%')
        self.douyin_dianzan_gailv_baifenhao_dianzan.setFixedWidth(50)
        self.h_layout_gailv_dianzan.addWidget(self.douyin_dianzan_gailv_dianzan)
        self.h_layout_gailv_dianzan.addWidget(self.dianzan_gailv_dianzan)
        self.h_layout_gailv_dianzan.addWidget(self.douyin_dianzan_gailv_baifenhao_dianzan)
        ...
        layout.addLayout(self.h_layout_gailv_dianzan)
    ...
    def save_config(self):
        try:
            ...
            updata_pkl_config_mianban("xiuxiduochangshijian", self.xiuxiduochangshijian.text())#dianzan_gailv_dianzan
            updata_pkl_config_mianban("dianzan_gailv_dianzan", self.dianzan_gailv_dianzan.text())  # dianzan_gailv_dianzan
