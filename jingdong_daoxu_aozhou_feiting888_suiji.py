    ...
    with lock:
        ...
        if (result_touzhu == "1"):
            ...
            if (fanhuishouye != None):
                ...
                #     time.sleep(1)
                #     # time.sleep(5)
def jianshuo(alldata,driver,touzhu,mingci_flag,touzhu_flag,jiekoudizhi,ex,feiting_dizhi,touzhu_zhengfan,zhengfan,wangqi,suijishuju):
    print("jiekoudizhi------------------------------->",jiekoudizhi)
    print("feiting_dizhi------------------------------->", feiting_dizhi)
    print("touzhu_zhengfan------------------------------->", touzhu_zhengfan)
    print("wangqi------------------------------->", wangqi)
    print("wangqi------------------------------->", suijishuju)
    start_mingci_flag = 0
    diyici_flag = 0
    shunxu_flag = 0
    ...
    result_suijishuju = generate_array(suijishuju)
    print("result_suijishuju=", result_suijishuju)
    luzi_zongshu = len(result_suijishuju)
    while (True):
            print("开始-------------------------脚本开始--------------------------")
        ...
        # try:
            ...
            if(int(kaijiangshijian)>200):
                ...
                result_feiting = getData_wangqi(1, int(mingcishuzu[mingcishuzu_index]),int(0), jiekoudizhi) #获取飞艇的结果
                #print("alldata---------------->",alldata)
                tz_flag = "shuang"
                zhengfan = result_suijishuju[luzi_dangqiancixu]
                print("zhengfan=",zhengfan)
                if(touzhu_flag == "单双"):
                ...
                if (dangqiancixu > len(touzhu) - 1):
                    dangqiancixu = 0
                if (luzi_dangqiancixu > len(result_suijishuju) - 1):
                    luzi_dangqiancixu = 0
                    result_suijishuju = generate_array(suijishuju)
                print("最后报账，，，")
                print("dangqiancixu:",dangqiancixu)
                print("luzi_dangqiancixu:", luzi_dangqiancixu)
...
def get_jiekou_shuju(jiekoudizhi, retries=3, delay=1):
    ...
    with lock11:  # 确保只有一个线程可以进行请求
        ...
        for attempt in range(retries):
            ...
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0."+str(random.randint(1,4000))+".124 Safari/537.36",
                "Cookie": "name=value; sessionid=1234567890abcdef"  # Cookie字符串
            }

            try:
                response = requests.get(jiekoudizhi,headers=headers)
                # 检查请求是否成功
...
def main_gui(path,touzhujine,shuzu,monidizhi,jiekoudizhi,feiting_dizhi,touzhu_zhengfan,suiji_luzi_shuzi):
    ...
    # 运行应用
    ex = ""
    print("shuzu----------",shuzu,suiji_luzi_shuzi)
    thread111 = threading.Thread(target=main_tz,args=(path,touzhujine,shuzu,monidizhi,jiekoudizhi,ex,feiting_dizhi,touzhu_zhengfan,suiji_luzi_shuzi))
    thread111.start()
    #sys.exit(app.exec_())
#
def main_tz(path,touzhujine,shuzu,monidizhi,jiekoudizhi,ex,feiting_dizhi,touzhu_zhengfan,suiji_luzi_shuzi):
    ...
    for temp in shuzu:
        ...
        if(len(str(result_alldata))>8):
            thread = threading.Thread(target=jianshuo, args=(result_alldata,driver,touzhujine, temp["mingci"], leixing_flag, jiekoudizhi,ex,feiting_dizhi,touzhu_zhengfan,temp["zhengfan"],temp["wangqi"],suiji_luzi_shuzi))
            thread.start()
