...
def operate_device(serial: str, search_path: str, comment_path: str, task: List[str],
                   run_time: str, change_small: str, chang_big: str, swipe_small: str,
                   swipe_big: str, swipe_count: str, shoucang: str, pinglun: str,
                   dianzan: str, guanzhu: str, shipinhuadongcishu: str, gouwu: str,
                   shifouguanbidouyin: bool, douyinguanjianzi: str, douyinsuosuoguanjianzi: str,
                   douyinshiyongguanjianzi: str, moshi: List[str], zhibojianshoucang: str,
                   zhibojianpinglun: str, zhibojiandengpai: str) -> None:
    """设备操作主循环"""
    count_zong = 0
    task_start_time = time.time()
    total_timeout = int(run_time) * 60 + 300  # 总超时时间
    ...
    while True:
        try:
            ...
            # 总超时检查
            if time.time() - task_start_time > total_timeout:
                print(f"设备 {serial} 任务超时")
                return

            # 执行主逻辑
            result = main(
                serial, search_path, comment_path, task, run_time, change_small, chang_big,
                swipe_small, swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,
                count_zong, shipinhuadongcishu, gouwu, douyinguanjianzi, douyinsuosuoguanjianzi,
                douyinshiyongguanjianzi, moshi, zhibojianshoucang, zhibojianpinglun, zhibojiandengpai
            )

            if result == "88":
                print(f"设备 {serial} 运行结束")
                filepath = f'./shuju/{serial}.pkl'
                if os.path.isfile(filepath):
                    updata_pkl(filepath, "执行状态", "运行结束")
                    updata_pkl(filepath, "进行的任务", "空闲")

                # 关闭抖音
                if shifouguanbidouyin:
                    print(f"关闭设备 {serial} 的抖音")
...
class PklViewer(QMainWindow):
    ...
    def bbbb(self, tasks:List[str], moshi:List[str]):
        """启动设备任务线程的入口方法"""
        # 限制最大并发线程数
        max_concurrent = 100
        ...
        for serial in self.selected_ids:
            # 更新设备状态为启动中
            device_pkl_path = f'./shuju/{serial}.pkl'
            if os.path.isfile(device_pkl_path):
                updata_pkl(device_pkl_path, "执行状态", "启动中")

            # 准备线程参数
            thread_args = (
                serial,
                self.file_textbox.text(),
                self.file_textbox_comment.text(),
                tasks,
                self.run_time.text(),
                self.line_edit_from_search.text(),
                self.line_edit_to_search.text(),
                self.line_edit_from.text(),
                self.line_edit_to.text(),
                self.jiarenshurukuang.text(),
                self.shoucang_gailv.text(),
                self.shoucang_pinglun.text(),
                self.shoucang_dianzan.text(),
                self.shoucang_guanzhu.text(),
                self.huadongcishu_big.text(),
                self.file_textbox_gouwu.text(),
                self.radio_button3.isChecked(),
                self.douyinguanjianzi.text(),
                self.douyinsousuoguanjianzi.text(),
                self.douyinshiyongguanjianzi.text(),
                moshi,
                self.shoucanggailv_dibahang.text(),
                self.pinglungailv_dibahang.text(),
            ...
            def thread_func(*args):
                with semaphore:
                    self.operate_device_wrapper(*args)

            # 创建并启动线程
            thread_name = f"Device-{serial}-Task-{'-'.join(tasks)}"
    ...
    def start_thread_monitor(self) -> None:
        """启动线程监控，定期检查线程状态并更新UI"""

        def monitor():
            while True:
                # 过滤已结束的线程
                active_threads = [t for t in self.running_threads if t.is_alive()]
                self.running_threads = active_threads

                # 打印当前活跃线程数
                print(f"当前活跃线程数：{len(active_threads)}")
    ...
    def operate_device_wrapper(self, *args) -> None:
        """线程包装器：处理异常、重试和状态管理"""
        serial = args[0] if args else "unknown"
        filepath = f'./shuju/{serial}.pkl'
        max_retry = 3
        retry_count = 0
        ...
        while retry_count < max_retry:
            try:
                # 更新状态为执行中
                if os.path.isfile(filepath):
                    updata_pkl(filepath, "执行状态", f"执行中（第{retry_count + 1}次尝试）")
                ...
                if os.path.isfile(filepath):
                    updata_pkl(filepath, "执行状态", "运行结束")
                    updata_pkl(filepath, "进行的任务", "空闲")
                print(f"设备 {serial} 任务完成（线程: {threading.current_thread().name}）")
                break

            except BaseException as e:
                retry_count += 1
                error_msg = f"设备 {serial} 异常（{retry_count}/{max_retry}）: {str(e)}"
                print(error_msg)
                pkl_add_log("log.pkl", f"Device-{serial}", error_msg)

                # 更新状态为重试中
                if os.path.isfile(filepath):
                    updata_pkl(filepath, "执行状态", f"重试中（{retry_count}/{max_retry}）")
            ...
            # 重试失败
            error_msg = f"设备 {serial} 达到最大重试次数，已终止"
            print(error_msg)
            pkl_add_log("log.pkl", f"Device-{serial}", error_msg)
            if os.path.isfile(filepath):
                updata_pkl(filepath, "执行状态", "执行失败")
                updata_pkl(filepath, "进行的任务", "空闲")

    def refresh_pkl_files(self) -> None:
        """刷新pkl文件显示（实际实现由UI框架决定）"""
        print("刷新设备状态显示")
    ...
    def get_device(self, serial: str) -> u2.Device:
        ...
        try:
            ...
            raise ConnectionError(f"设备 {serial} 连接失败: {str(e)}")
