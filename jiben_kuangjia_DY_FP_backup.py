    ...
    for i in range(500):
        ...
        if (d(text='更多').exists(timeout=3)):
            ...
            if(daoding_flag == 1):
                ...
                for more in mores:
                    back_to_orderList(d)
                    y = get_view_y_coordinate(more.info)
                    ...
                    # print(all_data)
                    wuliu_2_flag = 0
                    result_center = get_view_logistics_center(d.dump_hierarchy(), y)
                    if(result_center != None):
                        wuliu_2_flag = 1
                        xx,yy = result_center
                        d.click(xx,yy)
                        time.sleep(random.randint(1, 3))
                    ...
                    if (d(text='申请开票').exists(timeout=3)):
                        ...
                        if (str(result_info_tt).isdigit()):
                            ...
                            if (result_info_tt["订单编号"] != None):
                                back_to_orderList(d)
                                max_y = y  # 你可以改成你需要的数值
                                # 在这里设置你的 Y 坐标上限（只找这个坐标之上的元素）
                                # 过滤出 y <= max_y 的 ¥ 元素
                                # valid_elements = []
                                # for elem in d(text='查看物流'):
                                #     try:
                                #         elem_y = elem.info['bounds']['top']
                                #         if elem_y <= max_y:
                                ...
                                #
                                #     # 获取最后一个元素
                                #     last_more_aa = valid_elements[-1]
                                #
                                #     # ========== 修复：手动计算中心点，解决 KeyError: center_x ==========
                                #     bounds = last_more_aa.info['bounds']
                                #     left = bounds['left']
                                #     top = bounds['top']
                                #     right = bounds['right']
            ...
            if(dianpu_name == None ):
                ...
                continue
            wuliu_flag = 0
            if (d(text='查看物流').exists(timeout=3)):#查看物流有时候不是点击更多 的 弹窗，而是直接暴露出来的
                print("查看物流")
...
def back_to_orderList(d):
    print("开始执行返回")
    for i in range(10):
        if (d(text='待支付').exists(timeout=3)):
            print("返回有待支付")
            time.sleep(2)
            return 1
