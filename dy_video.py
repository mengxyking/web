...
video_lock_ = threading.Lock()

def get_random_pkl_file_in_directory_shunxu(directory, current_index=0):
    # 获取目录下所有 .pkl 文件的列表
    pkl_files = [f for f in os.listdir(directory) if f.endswith('.pkl')]
    print("pkl_files=", pkl_files)
    ...
    # 如果没有 .pkl 文件，则直接返回 False
    if not pkl_files:
        return False, 0  # 返回一个元组，包含结果和下一个索引

    with video_lock_:
        # 确保索引在有效范围内
        if current_index >= len(pkl_files):
            current_index = 0  # 循环回到开头

        # 顺序选择一个 .pkl 文件
        chosen_file = pkl_files[current_index]
        file_path = os.path.join(directory, chosen_file)
        ...
        try:
            ...
            if 'TONGJI' in data and 'BIG_COUNT' in data and isinstance(int(data['TONGJI']),
                ...
                if int(data['TONGJI']) < int(data['BIG_COUNT']):
                    # 返回选中的文件和下一个索引
                    return chosen_file, (current_index + 1) % len(pkl_files)
        except Exception as e:
            print(f"Error reading file {file_path}: {e}")

        # 如果没有文件满足条件，则返回 False 和下一个索引
        return False, (current_index + 1) % len(pkl_files)
...
def bbb(d,serial,jiarenshuliang,big):
    ...
    while (True):
        ...
        time.sleep(2)
        if (d(text="打开App").exists(timeout=3)):
            d(text="打开App").click()
        else:
