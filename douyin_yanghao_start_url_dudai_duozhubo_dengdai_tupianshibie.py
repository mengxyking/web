...
def operate_device(serial, class_phone,search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai,fudai_guanjianzi,jinrufangshi,shouyehuadongxiao,shouyehuadongda):
    result_j = judge()
    if(result_j == False):
        print("当前需要联系")
        return
    ...
    while(True):
        try:
            print("while------------")
            result = duozhubo(serial, class_phone,search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                          swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai,fudai_guanjianzi,jinrufangshi,shouyehuadongxiao,shouyehuadongda)
            if (result == "88"):
                print("运行结束")
            ...
            print("崩溃了",ee)
            operate_device(serial, class_phone,search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                           swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai,fudai_guanjianzi,jinrufangshi,shouyehuadongxiao,shouyehuadongda)
...
def duozhubo(serial, class_phone, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
             ...
             shifouguanbidouyin, guanzhuzhanghao, fudai,fudai_guanjianzi,jinrufangshi,shouyehuadongxiao,shouyehuadongda):
    if(len(fudai)>0):
        ...
        while(True):
            ...
            try:
                result_main = main(serial, class_phone, search_path, comment_path, task, run_time, change_small,
                                   ...
                                   fudai_list[i], fudai_guanjianzi,jinrufangshi,zhubo_len,shouyehuadongxiao,shouyehuadongda)
            except:
...
def find_id_from_area(d,x1,y1):
    ...
    if candidates:
        ...
        print("未找到符合条件的控件")
def main(serial, class_phone,search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai,fudai_guanjianzi,jinrufangshi,zhubo_len,shouyehuadongxiao,shouyehuadongda):
    new_flag = "older"
    print("fudai,fudai_guanjianzi=",fudai,fudai_guanjianzi)
    if(len(task) == 0):
        print("0")
        return
    print("1")
    ...
    if (str(class_phone).count("-") > 1):
        ...
        if(temp_time.isdigit()):
            ...
            print("sleep_time_phone=",sleep_time_phone)
    print("4")
    ...
    if ("fudai" in task):
        ...
        if(jinrufangshi == "通过关注进入"):
            ...
            if(new_flag == "newer"):
                if (d(descriptionContains="关注按钮").exists(timeout=3)):
                    print("关注按钮")
                    print(d(descriptionContains="关注按钮").info["bounds"]["bottom"])
                ...
                if (d(textContains=fudai).exists(timeout=15)):
                    ...
                    if (len(bb) > 1):
                        print("2")
                        bottom = bb[1].info["bounds"]["bottom"]
                        left = bb[1].info["bounds"]["left"]
                        print("bottom", bottom, left)
        ...
        fudai_view_id = get_value_by_key_pkl("shuju_config.pkl", "sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao")
        sousuocipinlvxiao_shouye_pinglunjiangeshijian_da = int(get_value_by_key_pkl("shuju_config.pkl", "sousuocipinlvxiao_shouye_pinglunjiangeshijian_da"))
            ...
            if(new_flag == "newer"):
                ...
                if(fudai_resourceId == None):
                    if (d(description="关闭").exists(timeout=3)):  # com.ss.android.ugc.aweme:id/yxl
                        ...
                        close_point_y = d(resourceId="com.ss.android.ugc.aweme:id/root").info["bounds"]["bottom"] + 180
                        print("close_point_y=",close_point_y)
                ...
                print("fudai_resourceId11=", fudai_resourceId)
                    ...
                    if (d(resourceId=fudai_resourceId).exists(timeout=0.1)):
                        ...
                        if(d(description="红包").exists(timeout=1)):
                            ...
                            if (d(descriptionContains="优惠券").exists(timeout=1)):
                                ...
                                d.click(temp_t["bounds"]["right"]+80,temp_t["bounds"]["top"]+50)
                            else:
                        ...
                        if (str(all_data).count("我知道了")>0):
                            ...
                            if (str(all_data).count("后开奖") > 0):
                                ...
                                if (len(points) > 0):
                                    ...
                                    continue
                                flag = 1
                            elif (str(all_data).count("等待开奖") > 0):
                                # d(text='添加评论...').click()
                                print("参与成功弹窗没有 自动隐藏")
                                points = ocr.getPoints_by_data(all_data, "后开奖")
                                if (len(points) > 0):
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.label_from_search_shouye_pinglunjiangeshijian = QLabel('  福袋控件ID')

        sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao = get_value_by_key_pkl("shuju_config.pkl", "sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao")
        if (sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao != None):
            self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao = QLineEdit(sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao)
        else:
            self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao = QLineEdit("yxl")
        self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao.setFixedWidth(100)

        self.label_to_search_shouye_pinglunjiangeshijian = QLabel('至')
        self.label_to_search_shouye_pinglunjiangeshijian.setFixedWidth(15)

        sousuocipinlvxiao_shouye_pinglunjiangeshijian_da = get_value_by_key_pkl("shuju_config.pkl", "sousuocipinlvxiao_shouye_pinglunjiangeshijian_da")
        if (sousuocipinlvxiao_shouye_pinglunjiangeshijian_da != None):
            self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_da = QLineEdit(sousuocipinlvxiao_shouye_pinglunjiangeshijian_da)
        else:
    ...
    def execute_button_clicked(self):
        ...
        updata_pkl_config_mianban("sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao", self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_xiao.text())
        updata_pkl_config_mianban("sousuocipinlvxiao_shouye_pinglunjiangeshijian_da",self.sousuocipinlvxiao_shouye_pinglunjiangeshijian_da.text())
    ...
    def thread_temp(self,tasks):
        for serial in self.selected_ids:
            ...
            thread = threading.Thread(target=operate_device, args=(serial,class_phone,self.file_textbox.text(),self.file_textbox_comment.text(),tasks,self.run_time.text(),self.line_edit_from_search.text(),self.line_edit_to_search.text(),self.line_edit_from.text(),self.line_edit_to.text(),self.jiarenshurukuang.text(),self.shoucang_gailv.text(),self.shoucang_pinglun.text(),self.shoucang_dianzan.text(),self.shoucang_guanzhu.text(),self.huadongcishu_big.text(),self.file_textbox_gouwu.text(),self.radio_button3.isChecked(),self.guanzhuzhanghao.text(),self.fudai.text(),self.fudai_guanjianzi.text(),self.combo_box.currentText(),self.line_edit_from_search_shouye.text(),self.line_edit_to_search_shouye.text()))
            #搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
