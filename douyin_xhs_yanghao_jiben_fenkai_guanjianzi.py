...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv,xiaohongshu_path,xiaohongshu_pinglun_path):
    count_zong = 0
    while(True):
        try:
            print("2")
            ...
            import time
                ...
                #return
            result = main(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv,xiaohongshu_path,xiaohongshu_pinglun_path)
            ...
            operate_device(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                           swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv,xiaohongshu_path,xiaohongshu_pinglun_path)
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv,xiaohongshu_path,xiaohongshu_pinglun_path):
    print("------=",serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv,xiaohongshu_path)
    print("task=",task)
    ...
    if("yanghao" in task):
        ...
        douyinguanjianzis = get_value_by_key_pkl("shuju_config.pkl", "tongchengguanjianzi_xhs")
        if (douyinguanjianzis):
            douyinguanjianzis = str(douyinguanjianzis).split("_")
        else:
            print("关键字配置的有问题")
            return
        while (b_count < swipe_count):
            ...
            if (random_boolean_with_probability(pinglun)):
                ...
                backTo_dy_detail(d)
                print("result_detail=", result_detail)
                if (result_detail == 1):
    ...
    if("yanghao_xiaohongshu" in task):
        while(True):
            ...
            xhs_shoucang_xiao = int(read_config("xhs_shoucang_xiao").strip())
            xhs_shoucang_da = int(read_config("xhs_shoucang_da").strip())
            xhs_dianzan_xiao = int(read_config("xhs_dianzan_xiao").strip())
            xhs_dianzan_da = int(read_config("xhs_dianzan_da").strip())
            xhs_pinglun_xiao = int(read_config("xhs_pinglun_xiao").strip())
            xhs_pinglun_da = int(read_config("xhs_pinglun_da").strip())

            shoucang = random.randint(xhs_shoucang_xiao,xhs_shoucang_da)
            dianzan = random.randint(xhs_dianzan_xiao, xhs_dianzan_da)
            pinglun = random.randint(xhs_pinglun_xiao, xhs_pinglun_da)
            print("shoucang=",shoucang,dianzan,pinglun)
            ...
            # d.app_start(package_name="ca.zgrs.clipper")
            # time.sleep(1)
            d.app_start(package_name="com.xingin.xhs", stop=False)
            backToHome_xhs(d)

            if (d(text='首页').exists(timeout=25)):
                print("当前在首页了。。。。。。。。")
            ...
            xhs_shanghua_liulan(d, serial,xiaohongshu_path, change_small, chang_big, xhshuadongliulangailv, dianzan, shoucang,
                                pinglun, xiaohongshu_pinglun_path, xhszuohuagailv, xhsshanghuagailv, swipe_count)
            backToHome_xhs_no_faxian(d)
            xhs_yanghao_search(d, xiaohongshu_path, change_small, chang_big, xhshuadongliulangailv, dianzan, shoucang,
                               pinglun, xiaohongshu_pinglun_path, xhszuohuagailv, xhsshanghuagailv)
            backToHome_xhs_no_faxian(d)
...
class PklViewer(QMainWindow):
    ...
    def init_xhs_tab(self):
        ...
        self.douyin_guanjianzi_wenben_xhs = QLabel('评论时间过滤:')
        self.douyin_guanjianzi_wenben_xhs.setFixedWidth(110)

        self.lang_combo = QComboBox()
        self.lang_combo.addItems(["小时", "一天内", "两天内", "时间不限"])
        ...
        self.douyin_guanjianzi_wenben_huadongcishu_xhs = QLabel('评论区域上划次数:')
        ...
        # self.h_layout_gailv_xhs.setSpacing(0)
        # self.h_layout_gailv_xhs.addStretch(1)
        #
        ...
        self.tab_widget.addTab(xhs_tab, "关键字相关配置")
