...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv):
    count_zong = 0
    while(True):
        #try:
            import datetime
            ...
            result = main(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                          swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv)
            if (result == "88"):
                print("运行结束")
        ...
        #                    swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv)
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv):
    print("------=",serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,xhszuohuagailv,xhsshanghuagailv,xhshuadongliulangailv)
    if(len(task) == 0):
        return
    d = get_device(serial)
    ...
    if("yanghao_xiaohongshu" in task):
        while(True):
            ...
            xhs_shanghua_liulan(d, serial,search_path, change_small, chang_big, xhshuadongliulangailv, dianzan, shoucang,
                                pinglun, comment_path, xhszuohuagailv, xhsshanghuagailv, swipe_count)
            backToHome_xhs_no_faxian(d)
            xhs_yanghao_search(d, search_path, change_small, chang_big, xhshuadongliulangailv, dianzan, shoucang,
                               pinglun, comment_path, xhszuohuagailv, xhsshanghuagailv)
            backToHome_xhs_no_faxian(d)
def read_config(key):
    from pathlib import Path
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        douyinguanjianzi_shanghua_xhs_liulan = get_value_by_key_pkl("shuju_config.pkl", "douyinguanjianzi_shanghua_xhs_liulan")
