...
import glob
import hashlib
from scripts.framework.vpc_utils import (
    parse_vless_url, get_vpc_groups, find_vpc_node,
    create_vpc_group_with_node, set_vpc_rule, set_vpc_domain_direct
)
...
class DeviceManageTool:
    ...
    def setup_ui(self):
        ...
        ttk.Label(net_frame, text="扫描网段(逗号分隔):").pack(side=tk.LEFT)
        self.network_entry = ttk.Entry(net_frame)
        ...
        self.device_tree.heading("容器名", text="容器名")
        self.device_tree.heading("主机IP", text="主机IP")
        ...
        self.device_tree.column("容器名", width=250)
        self.device_tree.column("主机IP", width=120)
        ...
        if region == "cell":
            ...
            for d in self.devices:
                if d["container_name"].lstrip("/") == container_name:
                    container_ip = d["ip"]
                    break
            key = (container_name, container_ip)
    ...
    def scan_devices(self):
        ...
        if networks_str:
            self.scan_networks = [n.strip() for n in networks_str.replace('，', ',').split(',') if n.strip()]
            
        self.scan_button.config(text="🔍 扫描中...")
        self.status_var.set("正在扫描设备...")
        self.log_message(f"开始扫描网段: {', '.join(self.scan_networks)}")
    ...
    def _scan_devices_thread(self):
        ...
        try:
            ...
            for network in self.scan_networks:
                ip_list.extend([f"{network}.{i}" for i in range(self.scan_start, self.scan_end + 1)])
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_ip = {executor.submit(self._scan_single_host, ip): ip for ip in ip_list}
                ...
                for future in as_completed(future_to_ip):
                    ...
                    try:
                        host_devices = future.result()
                        if host_devices:
                            temp_devices.extend(host_devices)
                    except Exception as e:
                        self.log_message(f"主机 {ip} 扫描异常: {e}")
                    
                    progress = (completed_hosts / total_hosts) * 100
                    self.root.after(0, lambda p=progress, c=completed_hosts, t=total_hosts, f=len(temp_devices): 
                        self.status_var.set(f"扫描进度: {p:.1f}% ({c}/{t}) - 发现 {f} 个设备"))
            
            if temp_devices:
                temp_devices.sort(key=self._device_sort_key)
                self.devices = temp_devices
                self.root.after(0, self._refresh_device_tree)
                self.log_message(f"✅ 扫描完成，发现 {len(temp_devices)} 个设备")
            else:
                self.devices = []
                self.root.after(0, self._refresh_device_tree)
                self.log_message("扫描完成，未发现任何设备")
    ...
    def _refresh_device_tree(self):
        ...
        for i, device_info in enumerate(sorted_devices, 1):
            container_name = device_info["container_name"].lstrip("/")
            key = (container_name, device_info["ip"])
    ...
    def _set_socks5_for_device(self, device, config):
        ...
        if proxy_type == "storm":
            ...
            self._setup_vpc(config, host_ip, container_name, log_prefix)
    ...
    def _setup_vpc(self, config, host_ip, container_name, log_prefix):
        """设置 VPC 节点和域名直连"""
        vpc_node = config.get("vpc_node", "")
        if not vpc_node or not host_ip:
            return

        self.log_message(f"{log_prefix} 正在设置VPC节点...")
        parsed = parse_vless_url(vpc_node)
        if not parsed:
            self.log_message(f"{log_prefix} ✗ VPC节点配置格式错误，无法解析VLESS链接")
            return

        vless_server = parsed.get("server", "")
        vless_port = parsed.get("port", 0)

        groups = get_vpc_groups(host_ip)
        if groups is None:
            self.log_message(f"{log_prefix} ✗ 获取VPC分组列表失败，无法设置VPC节点")
            return

        vpc_id, node_remarks = find_vpc_node(groups, vless_server, vless_port)

        if not vpc_id:
            self.log_message(f"{log_prefix} ⚠ 未找到匹配VPC节点，自动创建新分组...")
            group_alias = f"SK5-{vless_server}"
            vpc_id, node_remarks = create_vpc_group_with_node(host_ip, vpc_node, group_alias)
            if not vpc_id:
                self.log_message(f"{log_prefix} ✗ 自动创建VPC分组失败")
                return
            self.log_message(f"{log_prefix} ✓ 自动创建VPC分组成功 (vpcID={vpc_id})")

        success, msg = set_vpc_rule(host_ip, container_name.lstrip("/"), vpc_id, None)
        if success:
            self.log_message(f"{log_prefix} ✓ VPC节点设置成功 (vpcID={vpc_id}, remarks={node_remarks})")
            domain_filter = config.get("domain_filter", [])
            if domain_filter:
                doms = [str(d).strip() for d in domain_filter if str(d).strip()]
                if doms:
                    ok, count = set_vpc_domain_direct(host_ip, container_name.lstrip("/"), doms)
                    if ok:
                        self.log_message(f"{log_prefix} ✓ 域名直连设置成功 ({count}个域名，绕过SK5隧道)")
                    else:
                        self.log_message(f"{log_prefix} ✗ 域名直连设置失败")
        else:
            self.log_message(f"{log_prefix} ✗ VPC节点设置失败: {msg}")
