...
def get_app_page(app_biaoji, target_app):
    ...
    for item in app_biaoji:
        ...
        if app_name == target_app:
            return page_name
    return None
def into_app_home(d,index,item):
    for cc in range(3):
        print(f"cc={cc}")
        for i in range(3):
            print(f"i={i}")
            if (i == 0 and cc == 0 ):
                d.app_start(package_name=item,stop=True)
            else:
                d.app_start(package_name=item, stop=True)
            #----------------------进入首页---------------------------------------------------------------------------------------------------------------------
            if (index == "红果免费短剧" or index == "木叶免费短剧" or index == "番茄畅听" or index == "红果免费漫剧" or index == "番茄音乐" or index == "抖音火山版" or index == "悟空浏览器"  or index == "西瓜视频" ):
                ...
                result_back = backToHome_qiandao(d, 2, "阅读赚金币")
            else:
                result_back = backToHome_qiandao(d, 1, "首页")
        ...
        #--------------------开始点击入口进入--------------------任务页面--------------------------------
        print("--------------------开始点击入口进入--------------------任务页面--------------------------------")
        if (index == "红果免费短剧" or index == "红果免费漫剧" or index == "番茄音乐" or index == "番茄畅听" or index == "红果免费漫剧" or index == "木叶免费短剧" or index == "蛋花免费小说" or index == "常读免费小说" or index == "番茄免费小说"):
        ...
        if(index == "西瓜视频"):
            ...
            for bb in range(5):
                ...
                if (index == "木叶免费短剧" or index == "蛋花免费小说"  or index == "常读免费小说" or index == "番茄免费小说" or index == "红果免费漫剧" or index == "红果免费短剧" ):  # 如果有弹窗就直接点了
                    print("如果有弹窗就直接点了")
                ...
                app_biaoji = [{"红果免费短剧":"首页"},{"木叶免费短剧":"首页"},{"番茄畅听":"首页"},{"红果免费漫剧":"首页"},{"番茄音乐":"首页"},{"抖音火山版":"首页"},{"悟空浏览器":"首页"},{"西瓜视频":"首页"},{"汽水音乐":"发现"},{"蛋花免费小说":"书城"},{"常读免费小说":"书城"},{"番茄免费小说":"书城"},{"今日头条":"阅读赚金币"}]
                if(index == "今日头条"):
            ...
            for i in range(10): #开始滑动找签到按钮了
                print("需要 滑动页面找 领金币了")
                if (index == "木叶免费短剧" or index == "蛋花免费小说" or index == "常读免费小说" or index == "番茄免费小说" or index == "红果免费漫剧" or index == "红果免费短剧" ):
                    path_photo = take_screenshot(d)
                    all_data = ocr.yewu(path_photo)
                    if (str(all_data).count("签到领金币") > 0):
                        print("you签到领金币")
                    ...
                    if (str(all_data).count("立即签到") > 0):
                        ...
                        path_photo = take_screenshot(d)
                        all_data = ocr.yewu(path_photo)
                        if (str(all_data).count("去领取") > 0):
                            print("you立即签到88")
                            point = ocr.getPoint_by_data(all_data, "去领取")
                            if (point != None):
                                d.click(point[0], point[1])
                                time.sleep(5)
...
def main_douyin(serial):
    ...
    for app_name, package_name in simple_list:
        ...
        result_intoApp = into_app_home(d, app_name, package_name)
        if(result_intoApp != "1"):
            print("当前进入APP失败")
            continue
        time.sleep(3)
    ...
    # if (len(tasks) == 0):
    #     return
    # task_suiji_t = ["task-tuijian","task-qingli","task-tongcheng","task-huifu","task-sousuo"]
    # if (str(tasks).count("task-suiji") >0 ):
    #     tasks = random.sample(task_suiji_t, 3)
    #     print("无重复随机选择:", tasks)
    # random.shuffle(tasks)
    # print("顺序打乱之后的，组合=",tasks)
...
def beisaier_small(d,Diract="up"):
    ...
    if Diract == "up":
        ...
        random_start_point_y = random.uniform(0.5, 0.51)
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 操作区域标题和选择框
        self.caozuo_tiel = QLabel(" " * 70 + "途   不   停" + " " * 70)
