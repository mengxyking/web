            ...
            if(touzhu[dangqiancixu] == 0):
                ...
                while (True):
                    ...
                    if(temp_small != temp):
                        ...
                        result_2 = getData(count_0, int(mingcishuzu[mingcishuzu_index]),jiekoudizhi,touzhu_flag)
    ...
    if(str(alldata).count(mingci)>0):
        ...
        if(guanjun != None):
            ...
            if (daxiao == "shuang"):
                ...
                return "1"
            if (daxiao == "long"):
                pyautogui.moveTo(x=guanjun[0] + 56, y=guanjun[1] + 148)
                time.sleep(1.5)
                pyautogui.click()
                time.sleep(0.5)
                pyautogui.click()
                ...
                return "1"
            if (daxiao == "hu"):
                pyautogui.moveTo(x=guanjun[0] + 56, y=guanjun[1] + 178)
                time.sleep(1.5)
                pyautogui.click()
                time.sleep(0.5)
                pyautogui.click()
...
def getData(count,pos,jiekoudizhi,daxiao):
    import requests
    result = []
    # 接口的URL
    url = jiekoudizhi
    ...
    if response.status_code == 200:
        ...
        if((daxiao == "long") or (daxiao == "hu")):
            ...
            print("龙虎")
            print(longhu)
            result111.append(int(json.loads(data)["result"]["data"][small_count][longhu]))
        else:
            result111.append(int(json.loads(data)["result"]["data"][small_count]["preDrawCode"].split(",")[pos-1]))
            #small_count += 1
    return result111
#print(getData(1,5,"https://www.1689567.com/api/pks/getPksHistoryList.do?lotCode=10012","long"))
        ...
        if response.status_code == 200:
            ...
            if(str(data).count(str(riqi))>0):
                if((flag == "long") or (flag == "hu")):
                    ...
                    print("龙虎")
                    print(longhu)
                    temp = int(json.loads(data)["result"]["data"][0][longhu])
                    if(flag == "long"):
                        if(temp == 0):
                            return True
                        else:
