...
def main_control(serial):
    task = get_value_by_key_pkl(shuju_file, "task")
    print("task=",task)
    d = get_device(serial)
...
def main_douyin(serial,d):
    task = get_value_by_key_pkl(shuju_file, "task_douyin")
    print("task-----------",task)
    tasks = str(task).split("_")
    print("task=", tasks)
    if (len(tasks) == 0):
            ...
            if (zong):
                ...
                if (mumu_y - quanbu_y > 50):
                    ...
                    if (str(d.app_current()).count("NoteDetailActivity")):
                        ...
                        dianzan = int(get_value_by_key_pkl(shuju_file, "dianzan_gailv_xhs"))
                        shoucang = int(get_value_by_key_pkl(shuju_fileshuju_file, "shoucang_gailv_xhs"))
                        pinglun = int(get_value_by_key_pkl(shuju_file, "pinglun_gailv_xhs"))
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button1 = QCheckBox("抖音养号")
    ...
    def init_douyin_tab(self):
        ...
        douyinshipinguankanshichang_xiao = get_value_by_key_pkl(shuju_file, "douyinshipinguankanshichang_xiao")
