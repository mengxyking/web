...
from RapidOCR_json.api.python.demo_test import OCRProcessor
...
from RapidOCR_json.api.python import demo_test
...
def operate_device(serial,fromPage):
    print("1")
    ...
    while(True):
        #try:
            ...
            result = main_control(serial,fromPage)
            if (result == "88"):
                print("运行结束")
...
def main_control(serial,fromPage):
...
def guanggao(d,timeout_t=30):
    ...
    for i in range(5):
        ...
        all_data = demo_test.ocr_thread_worker(ocr,path_photo,time.time())
        print(all_data)
...
def into_app_home(serial , d,index,item):
    try:
        ...
        return "100"
def main_douyin(serial,fromPage):
    path = tongji(serial)
    ...
    task_renwu = get_value_by_key_pkl("shuju_config.pkl", "task")
    print("task_renwu=", task_renwu)
    if(1==1):


            print("111111111111111111111111")
            simple_list = [(list(item.keys())[0], list(item.values())[0]) for item in package_list]
        # if (str(task_renwu).count("douyinyanghao")):
        ...
        #     print("task1=", task)
            ...
            for app_name, package_name in simple_list:
                    ...
                    if(fromPage == "打开APP"):
                        cmd = f"adb -s {serial} shell am force-stop {package_name}"
                        # cmd_final = f'adb -s {serial} shell "pid=`{item}`; if [ ! -z $pid ]; then kill $pid; fi"'
                        cmd_final = f'adb.exe -s {serial} shell "pid=`pidof {package_name}`; if [ ! -z $pid ]; then kill $pid; fi"'
                        ...
                        print("点击签到")
                        #if (app_name == "红果免费短剧" or app_name == "木叶免费短剧"):
                        if(1==1):
                            print("红果免费短剧")
                            result_hongguo =hongguomianfeiduanju(d,app_name)
                            if(result_hongguo == "1"):
                                guanggao(d)
                    else:
                        print("")
                        print(d.app_current())
                        package_t = d.app_current()
                        print(package_t["package"])

                        target_value = package_t["package"]

                        # 遍历列表查找目标值
                        result = None
                        for index, item in enumerate(package_list):
                            # 遍历字典的键值对
                            for app_name, package_name in item.items():
                                if package_name == target_value:
                                    result = {
                                        "索引位置": index,
                                        "应用名称": app_name,
                                        "包名": package_name
                        ...
                        if result:
                            print(f"找到目标值：")
                            print(f"- 索引位置：{result['索引位置']}")
                            print(f"- 应用名称：{result['应用名称']}")
                            print(f"- 包名：{result['包名']}")
                        else:
                            print(f"未找到值为 '{target_value}' 的元素")
                            return

                        app_name = result['应用名称']
                        package_name = result['包名']
                        print(f"{app_name} -> {package_name}")
                        ...
                        all_data = demo_test.ocr_thread_worker(ocr, path_photo, time.time())
                        print(all_data)
                        jibi_int = ocr.getPoint_qiandao_jiahao(all_data)
        ...
        #                 all_data = demo_test.ocr_thread_worker(ocr,path_photo,time.time())
        #                 print(all_data)
        #                 jibi_int = ocr.getPoint_qiandao(all_data,30,900,100,700)
        #                 print("jibi_int=",jibi_int)
        #
        #                 jinbi_yuzhi = get_value_by_key_pkl("shuju_config.pkl", "huifuxiaoxiyonghunicheng")
        #                 create_file_if_not_exists(yunxing_txt_path)#huifuxiaoxiyonghunicheng
        ...
        #                 print("当前的广告没有正好进入啊")
        #             return "88"
        # if (str(task_renwu).strip().count("tongji")>0):
        #     print(d.app_current())
        #     package_t = d.app_current()
        #     print(package_t["package"])
        #
        ...
        #         append_to_txt(yunxing_txt_path, f"{app_name}签到异常，无法获取金币数量")
        #     return "88"
def hongguomianfeiduanju(d,index):
    for i in range(8):  # 开始滑动找签到按钮了
        print("需要 滑动页面找 领金币了")
        if (
                index == "木叶免费短剧" or index == "蛋花免费小说"  or index == "常读免费小说" or index == "番茄免费小说" or index == "红果免费漫剧" or index == "红果免费短剧" or index == "西瓜视频"):
            path_photo = take_screenshot(d)
            all_data = demo_test.ocr_thread_worker(ocr, path_photo, time.time())
            if (str(all_data).count("签到领金币") > 0):
                print("you签到领金币")
            ...
            if (str(all_data).count("立即签到") > 0):
                ...
                path_photo = take_screenshot(d)
                all_data = demo_test.ocr_thread_worker(ocr, path_photo, time.time())
                if (str(all_data).count("额外领") > 0):
                    print("额外领")
                    point = ocr.getPoint_by_datas(all_data, "额外领", "金币")
                    if (point != None):
                        d.click(point[0], point[1])
                        time.sleep(5)
                if (str(all_data).count("立即签到") > 0):
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button2 = QCheckBox("统计")
        ...
        self.execute_button = QPushButton("打开APP")
        self.execute_button.resize(80, 30)

        self.execute_button_tongji = QPushButton("执行统计")
        self.execute_button_tongji.resize(80, 30)

        # self.equit_button = QPushButton("打开APP")
        # self.equit_button.resize(80, 30)

        self.click_qiandao = QPushButton("点击签到")
        self.click_qiandao.resize(80, 30)

        self.click_tongji = QPushButton("点击统计")
        self.click_tongji.resize(80, 30)
        ...
        #self.button_gang.addWidget(self.execute_button_tongji)
        self.button_gang.addWidget(self.click_qiandao)
        self.button_gang.addWidget(self.click_tongji)
        ...
        # 绑定按钮事件
        self.execute_button.clicked.connect(lambda: self.execute_button_clicked("打开APP"))
        self.click_qiandao.clicked.connect(lambda: self.execute_button_clicked("点击签到"))
        self.click_tongji.clicked.connect(lambda: self.execute_button_clicked("点击统计"))
    ...
    def init_douyin_tab(self):
        ...
        self.label_file = QLabel("请选择Excel文件路径:")
        ...
        self.label_file_comment = QLabel("请选择Excel文件路径:")

        file_temp_path_comment = get_value_by_key_pkl("shuju_config.pkl", "file_path_comment2")
    ...
    def showDialog_comment(self):
        ...
        if dialog.exec() == QFileDialog.DialogCode.Accepted:
            ...
            if selected_files:
                ...
                updata_pkl_config_mianban("file_path_comment2", selected_file)
                #self.excel_file = selected_file
                #self.import_config()
                #self.refresh_pkl_files_test()
    ...
    def execute_button_clicked(self,fromPage):
        ...
        for serial in self.selected_ids:
            thread = threading.Thread(target=operate_device, args=(serial,fromPage,))
            #搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
    ...
    def execute_delete_button_clicked(self):
        ...
        print("port_t=", port_t)
        threading.Thread(target=self.lianjieshouji_thread,args=(file_path,port_t,)).start()
        #self.openExcel(file_path)
    def lianjieshouji_thread(self,file_path,port_t):
        #print("")
        result_excel = self.openExcel(file_path)
        ...
        count_c = 1000
        for data_t in result_excel:
            count_c += 1
            #print(data_t)
            if(str(data_t[0]).isdigit() and str(data_t[1]).count(".")>0):
                #print("--->")
                print(data_t[0],data_t[1],data_t[2])
                file_name = data_t[1]+":"+port_t
                print(file_name)

                # dic = {"name": f"{file_name}", "连接状态": "未知", "执行状态": "空闲中", "age": "1811", "add": "bj1",
                #        "xingbie": "nan", "进行的任务": "空闲", "nick_name": f"{str(count_c)}-{str(data_t[0])}-{str(data_t[2])}", "tongji": "0"}
                # pkl_add("./shuju/" + file_name + ".pkl", dic)

                if (file_name not in dict(sorted(pkl_list("config.pkl").items(), key=lambda item: item[1]))):
                    print("包含，但是也得改名字",file_name)
                updata_pkl_config("config.pkl", file_name, f"{str(count_c)}-{str(data_t[0])}-{str(data_t[2])}")


                threading.Thread(target=connect_port,args=(data_t[1],port_t)).start()
                time.sleep(0.01)
        print("手机导入完毕")
    ...
    def openExcel(self,filePath):
        ...
        # 1. 加载 Excel 文件（只读模式，提高效率）
        file_path = filePath
        wb = load_workbook(filename=file_path, read_only=True)

        # 2. 选择工作表
        ws = wb["Sheet1"]  # 按名称选择，或用 wb.active 获取当前激活的工作表

        # 3. 逐行读取数据
        print("表格所有数据：")
        all_data = []
        # 遍历每一行（iter_rows 可指定行范围，如 min_row=2 跳过表头）
        for row in ws.iter_rows(min_row=1, values_only=True):  # values_only=True 只获取值，不获取格式
            all_data.append(row)
            #print(row)  # 每行数据以元组形式输出，如 ('姓名', '年龄', '性别')
        ...
        #
        # # 方式2：按行列索引（行、列从1开始）
        # cell_2_3 = ws.cell(row=2, column=3).value
        # print("第2行第3列单元格值：", cell_2_3)
