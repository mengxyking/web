...
def run_script():
    ...
    try:
        ...
        if(checkbox222.isChecked() == True):
           dic111 = {"mingci":combo_box222.currentText(),"leixing":combo_box888.currentText()}
           shuzu.append(dic111)
        if (checkbox333.isChecked() == True):
            dic222 = {"mingci": combo_box333.currentText(), "leixing": combo_box3333.currentText()}
            shuzu.append(dic222)
        if (checkbox444.isChecked() == True):
            dic333 = {"mingci": combo_box444.currentText(), "leixing": combo_box4444.currentText()}
            shuzu.append(dic333)
        if (checkbox555.isChecked() == True):
            dic333 = {"mingci": combo_box555.currentText(), "leixing": combo_box5555.currentText()}
            shuzu.append(dic333)
        for temp in shuzu:
            print(temp)
        print("开始执行脚本")
        jingdong.main_gui(file_textbox.text(),start_textbox_kami.text(),shuzu,file_textbox_tz_dizhi.text(),file_textbox_jiekou_dizhi.text())
