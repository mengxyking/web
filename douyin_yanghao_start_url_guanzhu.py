...
#抖音养号+微信加好友脚本
shuju_config_file_name = "shuju_config_yanghao_guanzhu.pkl"
...
def main(serial):
    ...
    while(True):
        if( 1 == 1):
            ...
            if (d(descriptionContains='已选中，关注，按钮').exists(timeout=3)):
                print("当前在关注tab了")
            else:
            ...
            while (count_swipe < swipe_count):
                ...
                x11, y11 = d.window_size()
                d.swipe_points([(500, int(y11) - 400), (500, 200)], 0.2)
                time.sleep(random.randint(3,5))
                update_pkl_add_one("./shuju/" + str(serial) + ".pkl", "tongji")
                if (not  d(descriptionContains='点赞').exists(timeout=3) and d(descriptionContains='关注').exists(timeout=3)):
                    print("当前是费视频页面，可能是直播页面")
                    continue
                ...
                if(is_hit(zonggailv)):
                    ...
                    if (1 in res):
                        if (d(descriptionContains='点赞').exists(timeout=3)):
                            print("点击点赞")
                            if (d(descriptionContains='已点赞').exists(timeout=1)):
                                print("视频已点过赞")
                            else:
                    ...
                    if (2 in res):
                        if (d(descriptionContains='收藏').exists(timeout=3)):
                            print("点击收藏")
                            if (d(descriptionContains='已选中，收藏').exists(timeout=1)):
                                print("视频已点收藏")
                            else:
                    ...
                    if (3 in res):
                        ...
                        if (d(descriptionContains='评论').exists(timeout=3)):
                            result = comment(d, serial, comment_path)
                            if(result != "1"):
                                backToHome(d)
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        """)
        self.caozuo_config = QLabel("  抖音配置区")
        ...
        self.h_layout_diyihang.addWidget(QLabel('总概率  '))
