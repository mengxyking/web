...
def main(serial, task,comment_content,comment_fanwei,comment_count,comment_time,douyinsixinhuashu,tongbu_flag,tongchengdianzanzongshu,tongchengdianzandage):
    ...
    if("sixin" in task):
        sixin_douyin(d,serial,comment_content,comment_fanwei,comment_count,comment_time,comments)
...
def sixin_douyin(d,serial,comment_content,comment_fanwei,comment_count,comment_time,comments):
    ...
    while(pinglun_couont < count_all):
        ...
        if (d(className="android.widget.EditText").exists(timeout=10)):
            print("，按钮")
            # pingluns[-1].set_text(random.choice(comments))   className="android.widget.EditText"
            d(className="android.widget.EditText").set_text(random.choice(comments))
            time.sleep(1)
