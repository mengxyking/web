...
from bit_selenium import public_tk
...
def operate_device(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small, swipe_big,
                   swipe_count, shoucang, pinglun, dianzan, guanzhu, shipinhuadongcishu, gouwu, shifousixinzuozhe,
                   shifousixinpinglunyonghu, shifouzuixin, jinyiri, jinqiri):
    ...
    while (True):
        try:
            ...
            operate_device(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                           ...
                           shifousixinzuozhe, shifousixinpinglunyonghu, shifouzuixin, jinyiri, jinqiri)
...
def main(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small, swipe_big,
         swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong, shipinhuadongcishu, gouwu, shifousixinzuozhe,
         shifousixinpinglunyonghu, shifouzuixin, jinyiri, jinqiri):
    pass
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("tk")
        self.setGeometry(100, 100, 650, 300)
        # layout = QVBoxLayout()
        self.titleLabel = QLabel("*" * 55 + "任务列表" + "*" * 55)
        ...
        self.label_file = QLabel("                          请选择excel目录:")
        ...
        if (file_temp_path != None):
            ...
            self.file_textbox = QLineEdit("请选择excel目录")
    ...
    def on_file_button_clicked(self):
        ...
        if dialog.exec() == QFileDialog.DialogCode.Accepted:
            ...
            if selected_files:
                ...
                self.file_textbox.setText(selected_files[0])
                updata_pkl_config_mianban("file_path", selected_files[0])
    ...
    def thread_temp(self, tasks):
        for serial in self.selected_ids:
            thread = threading.Thread(target=operate_device,
                                      args=(serial, self.file_textbox.text(), self.file_textbox_comment.text()))
    ...
    def execute_delete_button_clicked(self):
        ...
        thread = threading.Thread(target=self.creat_video, args=())
        thread.start()
        self.refresh_pkl_files()
        self.selected_ids = []

    def creat_video(self):
        bit_selenium = public_tk()
        file_path = self.file_textbox.text()

        while(True):
            try:
                data = get_worker(file_path)
                print("data1----", data)

                if (data != None):
                    subprocess.run(['taskkill', '/F', '/IM', 'BitBrowser.exe'])
                    time.sleep(5)
                    res = bit_selenium.public_tk_m(data["比特浏览器key"], data["视频链接"], data["视频描述"],
                                                   data["商品ID"], data["发布时间"])
                    if (res == 1):
                        modify_excel(file_path, "任务状态", int(data["序号"]) + 1, 1)
                    time.sleep(10)
                else:
                    time.sleep(1 * 60)
            except:
...
def get_worker(file_name):
    """
    读取Excel文件并返回所有数据。

    :param file_name: Excel文件名
    :return: Excel表格中的数据（列表形式）
    """
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        print(str(dict(zip(headers, row))["发布时间"]) + "------fabushijian")
        print(str(dict(zip(headers, row))["任务状态"])+"------ooo")

        result_time = time_compare(str(dict(zip(headers, row))["发布时间"]))

        if(str(dict(zip(headers, row))["任务状态"]) != str(1) and result_time == True):
            return dict(zip(headers, row))

    return None

def time_compare(datatime):
    from datetime import datetime
    # 定义目标时间
    target_time = datetime.strptime(datatime, '%Y-%m-%d %H:%M:%S')
    # 获取当前时间
    current_time = datetime.now()
    # 比较时间并返回结果
    result = current_time > target_time
    print(result)  # 输出：True（因为当前时间是2025-03-25）
    return result
...
def update_cell(filename, column_name, row_number, new_value):
    """
    根据列名和行号修改Excel文件中的单元格内容。

    参数:
    filename (str): Excel文件的名称。
    column_name (str): 要修改的列的名称。
    row_number (int): 要修改的行的编号（从1开始）。
    ...
    # 找到列名对应的索引
    headers = [cell.value for cell in sheet[1]]
    column_index = headers.index(column_name) + 1  # 列索引从1开始
...
def modify_excel(file_name, column_name, row_num, new_value):
    """
    根据列名和行号修改Excel文件中的数据。

    :param file_name: Excel文件名
    :param column_name: 要修改的列名
    :param row_num: 要修改的行号（从1开始计数）
    :param new_value: 新的值
    """
    try:
        workbook = openpyxl.load_workbook(file_name)
        ...
        # 获取第一行以获取列名，并找到要修改的列的索引
        headers = [cell.value for cell in sheet[1]]
        if column_name not in headers:
            raise ValueError(f"Column '{column_name}' not found in the Excel file.")
        col_index = headers.index(column_name) + 1  # 列索引从1开始

        # 定位到指定的单元格并设置新值
        sheet.cell(row=row_num, column=col_index, value=new_value)
        ...
        print(f"The file '{file_name}' is currently open or locked by another program. Please close it and try again.")
