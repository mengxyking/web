    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                temps = line.split("+")
                if(len(temps)>1):
                    #if ((search_string == temps[0]) and (temps[1][0:2] == str(phone_num)[0:2] ) and (temps[1][len(temps[1])-2:len(temps[1])-1] == str(phone_num)[len(phone_num)-2:len(phone_num)-1] )):

                    # print("判断是----", ((search_string == temps[0]) and (temps[1][0:3] == str(phone_num)[0:3])))
                    # print("第一条判断(search_string == temps[0]=", (search_string == temps[0]))
                    # print("search_string=", search_string)
                    # print("temps[0]=", temps[0])
                    # print("temps[1]=", temps[1])
                    # print("temps[1][0:3]=", temps[1][0:3])
                    # print("str(phone_num)=", str(phone_num))
                    # print("str(phone_num)[0:3])=", str(phone_num)[0:3])
                    # print("str(temps[1][0:3]) == str(phone_num)[0:3]) = ", str(temps[1][0:3]) == str(phone_num)[0:3])
                    # print("search_string+aaaaa == temps[0])=",(search_string+"aaaaa" == temps[0]))
                    # print("and (str(temps[1][-2:]) == str(phone_num)[-2:]",(str(temps[1][-2:]) == str(phone_num)[-2:]))
