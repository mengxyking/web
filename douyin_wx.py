...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu):
    count_zong = 0
    while(True):
        try:
            result = main(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                          swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong,shipinhuadongcishu)
            if (result == "88"):
                print("运行结束")
            ...
            operate_device(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                           swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,shipinhuadongcishu)
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu):
    ...
    while(count_swipe<swipe_count):
        ...
        d.swipe_points([(500, 1500), (600, 400)], 0.2)
    ...
    time.sleep(3)
    if(language == 1):
        if (d(text='添加评论...').exists(timeout=3)):
            #d(text='添加评论...').click()
            print("当前在播放详情页里头")
            time.sleep(3)
            print("当前在播放详情页里头")
        ...
        if (d(text='Add comment...').exists(timeout=3)):
            #d(text='Add comment...').click()
            print("当前在播放详情页里头")
            time.sleep(3)
        else:
            print("当前没有Add comment a 。。。。。。。。")
            return
    ...
    while(b_count<swipe_count_temp):
        ...
        if (random_boolean_with_probability(pinglun)):
            print("当前可以pinglun")
            comment(d,language,serial,comment_path)
            time.sleep(1.5)
        ...
        if (check_time_difference(run_time * 60)):
            return "88"
        d.swipe_points([(500, 1200), (600, 400)], 0.2)
        time.sleep(2)
        b_count += 1
        if (check_time_difference(run_time * 60)):
            return "88"
...
def comment(d,language,serial,comment_path):
    ...
    if(language == 1):
        if(d(descriptionContains="发送评论").exists(timeout=2)):
            d(descriptionContains="发送评论").click()
    else:
        if (d(descriptionContains="Post comment").exists(timeout=2)):
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.radio_button2 = QCheckBox("微信加好友")
        ...
        self.label_file_douyin = QLabel("                     请选择微信手机号文件路径:")
        self.h_layout_dir_douyin.addWidget(self.label_file_douyin)

        file_temp_path = get_value_by_key_pkl("shuju_config.pkl", "file_path_douyin")
        if (file_temp_path != None):
            self.file_textbox_douyin = QLineEdit(file_temp_path)
        else:
            self.file_textbox_douyin = QLineEdit("请选择微信手机号文件路径")
        ...
        self.h_layout_dir_douyin.addWidget(self.temp_douyin)

        # 以下是评论文件选择器
        self.h_layout_dir_comment_douyin = QHBoxLayout()
        self.label_file_comment_douyin = QLabel("                       请选择微信备注文件路径:")
        self.h_layout_dir_comment_douyin.addWidget(self.label_file_comment_douyin)

        file_temp_path_comment_douyin = get_value_by_key_pkl("shuju_config.pkl", "file_path_comment_douyin")
        if (file_temp_path_comment_douyin != None):
            self.file_textbox_comment_douyin = QLineEdit(file_temp_path_comment_douyin)
        else:
            self.file_textbox_comment_douyin = QLineEdit("请选择微信备注文件路径")
        self.h_layout_dir_comment_douyin.addWidget(self.file_textbox_comment_douyin)
        self.file_button_comment_douyin = QPushButton("选择文件", self)
        self.temp_comment_douyin = QLabel("                          ")
        self.h_layout_dir_comment_douyin.addWidget(self.file_button_comment_douyin)
        self.h_layout_dir_comment_douyin.addWidget(self.temp_comment_douyin)


        self.label_from_douyin = QLabel('                 加好友间隔时间')
        self.line_edit_from_douyin = QLineEdit("10")
        self.line_edit_from_douyin.setFixedWidth(40)

        self.label_to_douyin = QLabel('至')
        self.label_to_douyin.setFixedWidth(15)
        self.line_edit_to_douyin = QLineEdit("30")
        self.line_edit_to_douyin.setFixedWidth(40)
        self.label_seconds_douyin = QLabel('秒内随机', self)

        self.label_from111_douyin = QLabel('  每天加好友上限')
        self.jiarenshurukuang_douyin = QLineEdit("8")
        self.jiarenshurukuang_douyin.setFixedWidth(25)
        # self.label_from222_douyin = QLabel('至')
        # self.label_from222_douyin.setFixedWidth(15)
        # self.huadongcishu_big_douyin = QLineEdit("20")
        # self.huadongcishu_big_douyin.setFixedWidth(25)
        self.label_fromci_douyin = QLabel('次')


        self.h_layout_douyin = QHBoxLayout()
        self.h_layout_douyin.addWidget(self.label_from_douyin)
        self.h_layout_douyin.addWidget(self.line_edit_from_douyin)
        self.h_layout_douyin.addWidget(self.label_to_douyin)
        self.h_layout_douyin.addWidget(self.line_edit_to_douyin)
        self.h_layout_douyin.addWidget(self.label_seconds_douyin)

        self.h_layout_douyin.addWidget(self.label_from111_douyin)
        self.h_layout_douyin.addWidget(self.jiarenshurukuang_douyin)
        # self.h_layout_douyin.addWidget(self.label_from222_douyin)
        # self.h_layout_douyin.addWidget(self.huadongcishu_big_douyin)
        self.h_layout_douyin.addWidget(self.label_fromci_douyin)

        self.h_layout_douyin.setSpacing(5)
        # 设置布局与窗口边框之间的边距（例如，设置为 0 像素）
        self.h_layout_douyin.setContentsMargins(0, 0, 0, 0)
        ...
        layout.addLayout(self.h_layout_dir_douyin)
        layout.addLayout(self.h_layout_kongge6)
        layout.addLayout(self.h_layout_dir_comment_douyin)
        layout.addLayout(self.h_layout_kongge7)
        layout.addLayout(self.h_layout_douyin)
        ...
        self.file_button_douyin.clicked.connect(self.showDialog_comment_douyin_phone_file)
        self.file_button_comment_douyin.clicked.connect(self.showDialog_comment_nick)
    ...
    def execute_button_clicked(self):
        ...
        for temp in self.selected_ids:
            ...
            updata_pkl("./shuju/" + temp + ".pkl", "进行的任务", "TIK Tok 养号中")
