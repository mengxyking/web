...
class LotteryLogic:
    def __init__(self):
        ...
        self.current_stake_index = 0  # 当前金额索引（与单双索引同步）
    ...
    def load_config(self):
        ...
        if os.path.exists("lottery_simple_config.json"):
            try:
                ...
                # 新增：校验单/双金额配置
                if "dan_str" in config:
                    self.log_signal.emit("info", f"【配置校验】单路金额(dan_str)原始配置：{config['dan_str']}")
                if "shuang_str" in config:
                    self.log_signal.emit("info", f"【配置校验】双路金额(shuang_str)原始配置：{config['shuang_str']}")
                return config
    ...
    def get_current_stake(self):
        ...
        try:
            ...
            if self.current_stake_index >= len(target_amounts):
                self.log_signal.emit("warning",
                                     f"{amount_type}金额索引{self.current_stake_index}超出列表长度{len(target_amounts)}，取最后一个值")
                return target_amounts[-1]

            current_amount = target_amounts[self.current_stake_index]
            self.log_signal.emit("info", f"【获取金额】{amount_type} | 索引{self.current_stake_index} → {current_amount}")
    ...
    def update_sequence_and_stake_index(self, win):
        ...
        if win:
            ...
            # 索引越界保护：超出50则循环到0（或固定在最后）
            if self.current_sequence_index >= 50:
                self.log_signal.emit("warning", f"序列索引{self.current_sequence_index}超出50，重置为0")
                self.current_sequence_index = 0
            # 金额索引越界保护（取单/双金额列表的最大长度）
            current_target = self.get_current_bet_target()
            max_stake_index = len(self.dan_amounts) if current_target == "单" else len(self.shuang_amounts)
            if max_stake_index > 0 and self.current_stake_index >= max_stake_index:
                self.log_signal.emit("warning",
                                     f"金额索引{self.current_stake_index}超出{current_target}路长度{max_stake_index}，固定在最后")
                self.current_stake_index = max_stake_index - 1
    ...
    def get_current_bet_target(self):
        ...
        try:
            ...
            if self.current_sequence_index >= len(self.bet_sequence):
                self.log_signal.emit("warning", f"序列索引{self.current_sequence_index}超出长度，取最后一个值")
                return self.bet_sequence[-1]
    ...
    def yewu(self, config_c=None):
        ...
        try:
            ...
            if not self.dan_amounts:
                ...
                self.log_signal.emit("warning", "单路金额(dan_str)配置无效，使用默认列表：[1,2,3,4,5,6,7,8,9,9,9,9,9]")
            self.log_signal.emit("info", f"解析单路金额(dan_str)成功：{self.dan_amounts}")
        except Exception as e:
            self.dan_amounts = [1, 2, 3, 4, 5, 6, 7, 8, 9, 9, 9, 9, 9]
            self.log_signal.emit("error", f"解析单路金额(dan_str)失败：{str(e)}，使用默认列表")
        ...
        try:
            ...
            if not self.shuang_amounts:
                ...
                self.log_signal.emit("warning", "双路金额(shuang_str)配置无效，使用默认列表：[1,2,3,4,5,6,7,8,9,9,9,9,9]")
            self.log_signal.emit("info", f"解析双路金额(shuang_str)成功：{self.shuang_amounts}")
        except Exception as e:
            self.shuang_amounts = [1, 2, 3, 4, 5, 6, 7, 8, 9, 9, 9, 9, 9]
            self.log_signal.emit("error", f"解析双路金额(shuang_str)失败：{str(e)}，使用默认列表")
        ...
        self.log_signal.emit("info", "2. 金额规则：单路取dan_str配置，双路取shuang_str配置，索引同步递进")
        self.log_signal.emit("info", "3. 赢局规则：重置序列/金额索引为0 + 重新生成50个单双序列")
        self.log_signal.emit("info", "4. 输局规则：序列/金额索引同步递进（超出则取最后值/循环）")
        self.log_signal.emit("info", "5. 初始投注：必须单值结果（单/双）才触发")
        self.log_signal.emit("info", "6. 赢局后续：仅单值结果投注；输局后续：单/多值均强制投注")
        ...
        if TEST_MODE:
            ...
            self.log_signal.emit("info", "- 金额规则：投注单取dan_str，投注双取shuang_str对应索引值")
