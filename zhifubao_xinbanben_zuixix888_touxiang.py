...
def add_account_while(d,serial,real_file_path,task,dir_path_shang,big,small,jiarenshuliang):
    ...
    while(count_temp<int(jiarenshuliang)):
        ...
        if(result_addfriend == 1):#成功
            update_pkl_add_one("./shuju/" + str(serial) + ".pkl", "tongji")

            dir_path_tongji = dir_path_shang + "/" + "有头像的"
            create_directory_if_not_exists(dir_path_tongji)
            file_path_tongji = dir_path_tongji + "/" +  "有头像的.txt"
            create_file_if_not_exists(file_path_tongji)

            with open(file_path_tongji, 'a', encoding='utf-8') as file:
                file.write(str(phone_temp).strip() + "\n")

        else:#失败

            dir_path_tongji = dir_path_shang + "/" + "没有头像的"
            create_directory_if_not_exists(dir_path_tongji)
            file_path_tongji = dir_path_tongji + "/" +  "没有头像的.txt"
            create_file_if_not_exists(file_path_tongji)
...
def add_big(d,serial,dir_path,ocr_processor,task,big,small,jiarenshuliang):
    ...
    if (d(resourceId='com.alipay.mobile.socialwidget:id/title_more_menu_button').exists(timeout=5)):
        elements = d(description="更多操作",
                     resourceId='com.alipay.mobile.socialwidget:id/title_more_menu_button')  # 获取所有文本为'some_text'的元素
        random_click_view(d, elements.info)
        time.sleep(0.1)
    else:
        return

    if (d(text="添加朋友", resourceId="com.alipay.mobile.antui:id/item_name").exists(timeout=5)):
    ...
    if (d(resourceId="com.alipay.mobile.antui:id/search_input_box").exists(timeout=5)):
...
def operate_device(serial, dir_path, task,big,small,jiarenshuliang):
    ...
    if ("add" in task):
        updata_pkl("./shuju/" + str(serial) + ".pkl", "进行的任务", "头像统计中")
        pkl_add_log("log.pkl", serial, "开始统计头像")
...
def addFriend(d,no,task,serial):
    ...
    if (d(resourceId="com.alipay.mobile.ui:id/social_search_normal_input").exists(timeout=8)):
    ...
    time.sleep(1)
    #d.press('enter')
    if (d(textContains="搜索",resourceId="com.alipay.mobile.contactsapp:id/leftText").exists(timeout=8)):
    ...
    if(d(text="你搜索的账号不存在，请更换手机号或邮箱后重新搜索").exists(timeout=2)):#如果账号不存在，则弹窗 点击确定按钮
    ...
    if (d(text="加好友").exists(timeout=5)):
        #elements = d(textContains="搜索",resourceId="com.alipay.mobile.contactsapp:id/leftText")  # 获取所有文本为'some_text'的元素

        if (d(textContains="提醒").exists(timeout=5)):
            # elements = d(textContains="搜索",resourceId="com.alipay.mobile.contactsapp:id/leftText")  # 获取所有文本为'some_text'的元素
            return
        else:
            return 1
    elif(d(text="转账").exists(timeout=3)):
        #elements = d(textContains="搜索",resourceId="com.alipay.mobile.contactsapp:id/leftText")  # 获取所有文本为'some_text'的元素
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.radio_button1 = QCheckBox("头像统计")
