...
def operate_device(serial):
    ...
    if (os.path.isfile(filepath)):
        ...
        updata_pkl(filepath, "进行的任务", "QQ任务")
...
class PklViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.radio_button1 = QCheckBox("QQ")
        ...
        self.execute_button = QPushButton("输入QQ号")
        ...
        self.execute_button_delete = QPushButton("输入验证码")
    ...
    def init_douyin_tab(self):
        ...
        self.label_file = QLabel("请选择QQ文件路径:")
        self.label_file.setFixedWidth(130)

        file_temp_path = load_from_json(json_file, "file_path")
        if (file_temp_path != None):
            self.file_textbox = QLineEdit(file_temp_path)
        else:
            self.file_textbox = QLineEdit("请输入QQ文件路径")
        ...
        self.label_file_comment = QLabel("请选择验证码文件路径:")
        self.label_file_comment.setFixedWidth(130)
        file_temp_path_comment = load_from_json(json_file, "file_path_comment111111")
        if (file_temp_path_comment != None):
            self.file_textbox_comment = QLineEdit(file_temp_path_comment)
        else:
            self.file_textbox_comment = QLineEdit("请选择验证码文件路径")
        ...
        #下面是两个大的输入框
        QQ_edit_input = load_from_json(json_file, "QQ_edit_input")
        print("")
        self.QQ_edit_input = QTextEdit()
        print("QQ_edit_input=",QQ_edit_input)
        if QQ_edit_input == None or QQ_edit_input == "":
            print("1")
            self.QQ_edit_input.setPlainText("QQ----密码----手机")
        else:
            print("2")
            self.QQ_edit_input.setPlainText(QQ_edit_input)
        self.QQ_edit_input.setFixedSize(300,200)

        # 第二个输入框：抖音视频观看时长（小）→ 你可以自己改名
        self.yanzhengma_edit = QTextEdit()
        yanzhengma_edit = load_from_json(json_file, "yanzhengma_edit")
        if yanzhengma_edit == None or yanzhengma_edit == "":
            self.yanzhengma_edit.setPlainText("手机号---验证码")
        else:
            self.yanzhengma_edit.setPlainText(yanzhengma_edit)
        self.yanzhengma_edit.setFixedSize(300,200)

        # 把两个输入框放进一行，紧挨着
        self.duration_layout = QHBoxLayout()
        self.duration_layout.setSpacing(0)  # 间距为0，完全挨在一起
        self.duration_layout.addWidget(self.QQ_edit_input)
        self.duration_layout.addWidget(self.yanzhengma_edit)

        self.label_left = QLabel("QQ配置")
        self.label_right = QLabel("验证码配置")
        self.label_left.setFixedWidth(300)
        self.label_right.setFixedWidth(300)
        self.wrap_layout = QHBoxLayout()
        self.wrap_layout.addWidget(self.label_left)
        self.wrap_layout.addWidget(self.label_right)
        ...
        # layout.addLayout(self.h_layout_dir_comment)#self.duration_layout
        layout.addLayout(self.wrap_layout)
    ...
    def save_config(self):
        try:#"yanzhengma_edit") #QQ_edit_input


            save_to_json(json_file,"QQ_edit_input", self.QQ_edit_input.toPlainText())
            save_to_json(json_file,"yanzhengma_edit", self.yanzhengma_edit.toPlainText())
    ...
    def execute_button_clicked(self):
        ...
        QQs = self.QQ_edit_input.toPlainText()

        line_list = QQs.splitlines()

        # 3. 去掉空行（可选，推荐加上）
        line_list = [line.strip() for line in line_list if line.strip()]

        # 现在 line_list 就是你要的数组
        print(line_list)
        for tt in line_list:
            print(tt)
        ...
        count_phont = 0
        for serial in self.selected_ids:
            if(count_phont >= len(line_list)):
                print("没有QQ号码了")
                return
            qq_no = line_list[count_phont]
            save_to_json(json_fileQQ,serial,qq_no)
            count_phont += 1
            thread = threading.Thread(target=self.input_QQ, args=(serial,qq_no,))
            #搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
    ...
    def add_text(self):
        print("")
    def input_yanzhengma(self,serial):
        d = u2.connect(serial)

        qq = load_from_json(json_fileQQ,serial,"")
        print("qq=",qq)
        if(not str(qq).count("----")):
            return
        ...
        print("1111")
        yanzhengmas = self.yanzhengma_edit.toPlainText()

        line_list = yanzhengmas.splitlines()
        ...
        if(str(yanzhengma).count("---")):
            yanzhengma_tt = str(yanzhengma).split("---")[1]
            if(str(yanzhengma_tt).isdigit()):
                shell_neibu(f"adb -s {serial} shell input text {yanzhengma_tt}")
            else:
                print("9090901")



    def input_QQ(self,serial,qq_no):
        #d = u2.connect(serial)
        print("1111")
        print(qq_no)
        if(str(qq_no).count("----")):
            print("baohan ----")
            qqqqqq = str(qq_no).split("----")[0]
            if(str(qqqqqq).isdigit()):
                print("QQ 号符合需求")
            else:
                print("QQ 号不符合要求")
                return

            # if(d(className="android.widget.RelativeLayout").exists(timeout=12)):#---
            #     print("zhaodao控件了")
            #     d(className="android.widget.RelativeLayout").click()
            #     time.sleep(1)
            #     cmd = f"adb -s {serial}  shell input text {qqqqqq}"
            #     shell_neibu(cmd)
            # else:
            #     print("没找到控件")
            #     d.click(500,680)
            ...
            if(1 == 1):#---
                ...
                cmd = f"adb -s {serial} shell input tap 500 680"
                shell_neibu(cmd)
                time.sleep(0.5)
                cmd = f"adb -s {serial}  shell input text {qqqqqq}"
                shell_neibu(cmd)
    ...
    def execute_delete_button_clicked(self):
        ...
        if (self.selected_ids == []):
            toast("请选择机型")
            return
        self.selected_ids.sort(key=lambda s: s.lower())
        ...
        for serial in self.selected_ids:
            ...
            save_to_json(json_fileQQ, serial, qq_no)
            count_phont += 1
            thread = threading.Thread(target=self.input_yanzhengma, args=(serial,))
            # 搜索路径、评论文件路径、任务列表、运行时长、更换频率小、更换频率大、视频滑动间隔小、视频滑动间隔大、视频滑动次数、收藏、评论、点赞、关注
            # threads.append(thread)
            thread.start()
...
def judge():
    ...
    if (os.path.isfile(final_str)):
        ...
        return False
def get_value_by_key_pkl_tt(file_path, key):
    try:
...
def create_hline():
    ...
    return line
# 保存数据到 json
def save_to_json(file_path, key, value):
    data = {}
    ...
    data[key] = value  # 自动保存换行 \n

    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
# 从 json 读取数据
def load_from_json(file_path, key, default=""):
    if not os.path.exists(file_path):
        return default
    try:
        with open(file_path, "r", encoding="utf-8") as f:
...
def save_value_to_pkl(file_path, key, value):
    try:
        data = {}
        with open(file_path, 'rb') as f:
            data = pickle.load(f)
...
if __name__ == "__main__":
    if(os.path.isfile(json_fileQQ)):
        os.remove(json_fileQQ)
