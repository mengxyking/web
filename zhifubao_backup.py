...
def delete(d, serial, dir_path, param, task, big, small, jiarenshuliang):
    ...
    while(count_while_temp < 15):
        ...
        if (d(resourceId='com.alipay.mobile.socialwidget:id/item_date').exists(timeout=3)):
            elements = d(resourceId='com.alipay.mobile.socialwidget:id/item_date')  # 获取所有文本为'some_text'的元素
            print("日期个数为：", len(elements))
            flag = 0

            if(len(elements) == len(red_point_list)):
                return "1"
            for ee in elements:
                print(ee.info["bounds"]["top"])
                result = is_close_to_any(int(ee.info["bounds"]["top"]),red_point_list)
            # for point in red_point_list:
            #     print(ee.info["text"])
            #     print("距离是：",int(ee.info["bounds"]["top"])-point)
                if(result):
                    print("当前这个有红点")
                    print(ee.info)
                else:
                ...
                #     print("当前一个红点没有了，可以删除")
                #     print(ee.info)
                #     flag = 1
                #     delete_frind_entry(d, serial, ee)
                print("最wai层flag=", flag)
                if(flag == 1):
                    print("中间层退出=")
                    break
    ...
    while (True):
        if (d(resourceId='com.alipay.mobile.socialwidget:id/red_dot_alert').exists(timeout=3)):
            ...
            if (d(resourceId='com.alipay.mobile.antui:id/title_text').exists(timeout=3)):
                ...
                # if (d(resourceId='com.alipay.mobile.chatapp:id/chat_msg_avatar_layout').exists(timeout=3)):
                #     d(resourceId='com.alipay.mobile.chatapp:id/chat_msg_avatar_layout').click()
                #     time.sleep(2)
                #
                #     if (d(resourceId='com.alipay.android.phone.wallet.profileapp:id/tv_right').exists(timeout=3)):
                #         phone_num = d(resourceId='com.alipay.android.phone.wallet.profileapp:id/tv_right').get_text()
                ...
                file_path_tongji = dir_path_tongji + "/" +get_value_by_key_pkl(pkl_file_path,str(serial)) +"_"+serial + ".txt"
                create_file_if_not_exists(file_path_tongji)

                file_path_done = dir_path + "/" + "申请添加成功的联系人" + "/" +get_value_by_key_pkl(pkl_file_path,str(serial)) +"_"+ serial + ".txt"
                if (not os.path.isfile(file_path_done)):
                    print("当前页面的配置文件还没有呢")
                    return

                success_string = find_string_in_file(file_path_done, title_text)
...
if __name__ == "__main__":
    ...
    #operate_device("Q5S0219527003267",r"C:\Users\Administrator\Desktop\config\需要添加的联系人\12_Q5S0219527003267.txt","")