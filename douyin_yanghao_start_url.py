...
def operate_device(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai):
    count_zong = 0
    while(True):
        try:
            result = main(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                          swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu, count_zong,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai)
            if (result == "88"):
                print("运行结束")
            ...
            operate_device(serial, search_path, comment_path, task, run_time, change_small, chang_big, swipe_small,
                           swipe_big, swipe_count, shoucang, pinglun, dianzan, guanzhu,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai)
...
def main(serial, search_path,comment_path, task,run_time,change_small,chang_big,swipe_small,swipe_big,swipe_count,shoucang,pinglun,dianzan,guanzhu,count_zong,shipinhuadongcishu,gouwu,shifouguanbidouyin,guanzhuzhanghao,fudai):
    if(len(task) == 0):
        return
    ...
    while(True):
        if ("guanzhu" in task):
            ...
            if (d(resourceId='com.ss.android.ugc.aweme:id/et_search_kw').exists(timeout=15)):
                d(resourceId='com.ss.android.ugc.aweme:id/et_search_kw').set_text(guanzhuzhanghao)
                time.sleep(3)
            else:
                print("当前没有搜索框。。。。。。。。")
                return
            ...
            if (d(textContains=guanzhuzhanghao).exists(timeout=15)):
                bb = d(textContains=guanzhuzhanghao)
                if(len(bb)>1):
                    bb[1].click()
                    time.sleep(3)
            ...
            if (d(text="已关注").exists(timeout=3)):
                #d(text="已关注").click()
                time.sleep(4)
                ...
                print("filepath-->", filepath)
                if (os.path.isfile(filepath)):
                    updata_pkl(filepath, "进行的任务", "关注任务完毕")
        ...
        if("yanghao" in task):
            ...
            if (1 == 0):
                ...
                if(str(d.app_current()).count("com.ss.android.ugc.aweme") > 0):
                    print("不需要重新启动app")
                else:
                    print("重新启动app")
                    d.app_start(package_name="com.ss.android.ugc.aweme")
            ...
            if (d(text='首页').exists(timeout=15) or d(text='推荐').exists(timeout=15)):
                print("当前在首页了。。。。。。。。")
            else:
                print("当前bu在首页了。。。。。。。。")
                return
            ...
            if (check_time_difference(run_time * 60)):
                print("222")
                return "88"
            if (d(text='首页').exists(timeout=3) or d(text='推荐').exists(timeout=3)):
                print("333")
                d.click(d.info["displayWidth"] - 50, 180)
                time.sleep(5)
            else:
        ...
        if("gouwu" in task):
            bbbb = 0
            gouwu = get_random_line_from_file(gouwu)
            cmd = f"adb -s {serial} shell am start -a android.intent.action.VIEW -d {str(gouwu).strip()}   -n com.ss.android.ugc.aweme/.app.DeepLinkHandlerActivity"
            while(bbbb < 10):
                shell_neibu(cmd)
                if (d(text="加入购物车").exists(timeout=20)):
                    break
                if(bbbb>8):
                    return "99"
                bbbb+=1
            if (d(text="加入购物车").exists(timeout=1)):
                x1,y1,x2,y2 = d(text="加入购物车").bounds()
                d.click(d.info["displayWidth"] - 50,y2-50)
                time.sleep(3)
                return '99'
            else:
                return '99'
...
class PklViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.label_from111_guanzhu = QLabel('关注账号名称:')
        self.label_from111_guanzhu.setFixedWidth(50)
        ...
        if (shoucanggailv != None):
            ...
            self.guanzhuzhanghao = QLineEdit("广东夫妇")
        self.guanzhuzhanghao.setFixedWidth(150)
        self.label_from222_guanzhu = QLabel('')

        self.label_from111_fudai = QLabel('福袋配置:')
