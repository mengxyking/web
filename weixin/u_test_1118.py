...
def register(device,row1,ocr):
    ...
    try:
        ...
        for i in range(10):
            if (d(text="去驗證").exists(timeout=5)):
                # d(text="关注").click()
                print("quyanzheng")
...
def jiaoyanma(device):
    ...
    foreground = d.app_current()
    print("foreground=",foreground)
    pkg_name = foreground["package"]
    activity_name = foreground["activity"]
    print("pkg_name=", pkg_name)
    print("activity_name=", activity_name)
    d.app_start(pkg_name, stop=False)  # stop=False 不关闭现有页面
    time.sleep(3)
    print("------------------------------------------")
    print(d.dump_hierarchy())
    d.stop_uiautomator(wait=True)
    ...
    for i in range(10):
        if (d(textContains = "無障礙方式").exists(timeout=3)):
            # d(text='添加评论...').click()
            print("無障礙方式")
            time.sleep(8)
...
def check_bank_cark(device,visa_ThreadSafeExcelHandler,ThreadSafeExcelHandler,list_shuju,row_temp):
    ...
    for i in range(10):
        if (d(textContains="繼續註冊").exists(timeout=5)):
            ...
            time.sleep(5)
            print("you 继续注册")
            temp_tt = d(textContains="繼續註冊").info
            random_click_view(d, temp_tt)
            time.sleep(1)
            random_click_view(d, temp_tt)
            time.sleep(1)
            random_click_view(d, temp_tt)
            ...
            time.sleep(1)
            d = u2.connect(device)
        elif(d(textContains="驗證請求無效").exists(timeout=3)):
    ...
    if (d(textContains="請填寫驗證碼").exists(timeout=15)):
        # d(text="关注").click()
        print("you 請填寫驗證碼")
    ...
    if (d(textContains="提交").exists(timeout=15)):
        # d(text="关注").click()
        print("you 提交")
        random_click_view(d, d(textContains="提交").info)
        time.sleep(3)
    else:
        print("11")
        return "66"
    ...
    # # print(alldata)
    # # d.click(alldata[0], alldata[1])
    # if (str(all_data).count("取消") > 0):
    #     # d(text='添加评论...').click()
    #     print("取消")
    #     point = ocr.getPoint_by_data(all_data, "取消")
    #     if (len(point) > 0):
    #         d.click(point[0], point[1])
    #         time.sleep(3)
    # else:
    ...
    # for temp in editTexts:
    #     temp.set_text("111")

    # d.shell(f"adb -s {device} shell input text "+str(4111))
    # time.sleep(3)


ocr = OCRProcessor()#網頁無法使用
devices = ["192.168.11.163:5011","192.168.11.163:5012","192.168.11.163:5013","192.168.11.163:5014","192.168.11.163:5015"]
#devices = ["192.168.11.163:5011"]
...
def yewu_most(device,ThreadSafeExcelHandler,ocr):
    ...
    if (result_jiaoyan != "1"):
        ...
        return "66"
    result_card = check_bank_cark(device,visa_ThreadSafeExcelHandler,ThreadSafeExcelHandler,list_shuju,row_temp)
    if(result_card == "2"):
