...
def main_douyin(serial):
    ...
    for i in range(1000):
        if(error_couont > 10):
            return 88
        try:
            result_yewu = zhuyao_yewu(d,serial ,shoujihao_file, erweima_file, erweima_suc_max,
                                      w)  # 0 代表一般错误，可以继续循环运行的，1代表成功，88代表 直接退出的  77代表需要多试几次的
...
def kill_all_wechat(DEVICE_SERIAL):
    ...
    subprocess.run(cmd, shell=True)
def zhuyao_yewu(d,serial,shoujihao_file,erweima_file,erweima_suc_max,w):



    result_excel_erweima = excel_read_w.get_next_valid_row(shoujihao_file,erweima_suc_max)
    print(result_excel_erweima)
    ...
    if (point_ruhuiyuan != None ):
        print("d当前是入会员")
        excel_read_w.modify_data(shoujihao_file, int(hanghao), "状态", "如会员")
        #return 1
    ...
    return 1
def error_process(d,shoujihao_file):
...
class PklViewer(QMainWindow):
    ...
    def init_douyin_tab(self):
        ...
        self.douyin_guanjianzi_wenben_tongcheng = QLabel('二维码成功注册个数:')
        ...
        self.douyin_guanjianzi_wenben_huifu = QLabel('API前缀配置:')
