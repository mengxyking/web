...
time_TT = 0
danshuang_TT = ""
# 新增：记录上一局实际开奖结果（用于解析跟/反）
last_actual_result = None


def extract_continuous_single_double_prefix(text):
    """
    提取字符串开头连续的“单/双”文字（适配“双双52”“单单8”“单双10”等场景）
    :param text: 输入字符串，如 "双双52"、"单单8"、"单双10"、"双52"
    :return: 连续的“单/双”组合字符串，无匹配则返回空字符串
    """
...
def extract_number_from_string(text):
    """
    从字符串中提取数字（支持整数、多位数）
    :param text: 输入字符串，如 "双52"、"单5"、"abc123def45"
    :return: 提取到的数字（int类型），若未找到数字返回 None
    """
...
# 新增核心函数：解析跟/反为实际单双值
# def parse_genfan_to_single_double(genfan_str, last_result):
#     """
#     解析跟/反为实际的单/双值
#     :param genfan_str: 跟/反字符串（如"跟"、"反"）
#     :param last_result: 上一局实际开奖结果（单/双）
#     :return: 解析后的单/双值，解析失败返回None
#     """
#     if not last_result or last_result not in ["单", "双"]:
#         return None
#
#     genfan_str = genfan_str.strip()
#     if genfan_str == "跟":
#         return last_result  # 跟：和上一局结果相同
#     elif genfan_str == "反":
#         return "双" if last_result == "单" else "单"  # 反：和上一局结果相反
#     else:
#         return None  # 非跟/反返回None

# 新增核心函数：解析跟/反为实际单双值
# 【这次 100% 按你要求写对，不反了】
def parse_genfan_to_single_double(genfan_str, last_result):
    print(f"genfan_str={genfan_str}, last_result={last_result}")
    last_result = last_result[:1]
    print("切完之后的last_result=",last_result)
    """
    解析跟/反为实际的单/双值
    【最终正确版】
    :param genfan_str: 跟/反字符串（如"跟"、"反"）
    ...
    if genfan_str == "跟":
        ...
        # 上一把是双 → 反 = 单
        # 上一把是单 → 反 = 双
        print("----->","单" if last_result == "双" else "双")
        return "单" if last_result == "双" else "双"
...
class LotteryLogic:
    def __init__(self):
        ...
        self.last_round_result = "输"  # 记录上一轮输赢结果
        self.initial_bet_triggered = False  # 标记初始投注是否已触发
        self.bet_history = []  # 投注历史记录

        # 单双序列相关
        self.bet_sequence = []  # 生成的50个单双方案列表
        self.current_sequence_index = 0  # 当前使用的单双序列索引
        # 单/双金额列表
        self.dan_amounts = []  # 单路金额列表
        self.shuang_amounts = []  # 双路金额列表
        self.current_stake_index = 0  # 当前金额索引
        ...
        # 新增：记录上一局实际开奖结果（用于解析跟/反）
        self.last_actual_lottery_result = None
    ...
    def run(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                CHECK_SECOND = bbb.get("check_second_jiange", 60 * 30)
                # 初始化变量
                a = -1
                b = -1
                refresh_interval = CHECK_SECOND  # 刷新间隔：2分钟（120秒）
                ...
                for i in range(1000000000000):
                    ...
                    if shuzi and shuzi > 50:
                        ...
                        time_TT = shuzi
                        # 修复点1：立即更新全局last_actual_result和实例变量
                        last_actual_result = danshuang_TT
                        self.last_actual_lottery_result = danshuang_TT
                        print(f"📢 数字大于50，记录开奖结果：{self.last_actual_lottery_result} | 全局记录：{last_actual_result}")
    ...
    def generate_bet_sequence(self, danshuang1, danshuang2, danshuang3, danshuang4, danshuang5, danshuang6):
        """
        生成50个单双的投注序列（核心函数，支持跟/反解析）
        规则：
        1. 以danshuang1为起点，后面按规则拼接danshuang2-6的内容，直到总长度≥50
        2. danshuang2-6为空字符串时直接过滤，不加入候选序列
        3. 随机选取候选序列时，选中过的序列暂不重复选，全部取完后重新循环
        4. 支持解析跟/反为实际单双值（依赖上一局开奖结果）
        """
        try:
            # 拆分各单双配置为列表，并过滤空值和非单/双/跟/反的内容
            def parse_seq(seq_str):
                """辅助函数：解析单双/跟反序列字符串，处理空值"""
                if not seq_str or seq_str.strip() == "":  # 处理空字符串情况
                    return []
                # 支持单、双、跟、反
                return [p.strip() for p in seq_str.split(',') if p.strip() in ["单", "双", "跟", "反"]]

            seq1 = parse_seq(danshuang1)
            seq2 = parse_seq(danshuang2)
            seq3 = parse_seq(danshuang3)
            seq4 = parse_seq(danshuang4)
            seq5 = parse_seq(danshuang5)
            seq6 = parse_seq(danshuang6)

            # 验证基础序列有效性
            if not seq1:
                self.log_signal.emit("error", "danshuang1配置无效（无单/双/跟/反值），默认使用['单']")
                seq1 = ["单"]

            # 收集非空的候选序列（处理danshuang2-6为空的情况）
            candidate_seqs = [seq2, seq3, seq4, seq5, seq6]
            candidate_seqs = [s for s in candidate_seqs if s]  # 过滤空列表
            if not candidate_seqs:
                self.log_signal.emit("warning", "danshuang2-6均无效，仅使用danshuang1填充")
                candidate_seqs = [seq1]

            # 生成50长度的序列（核心逻辑：支持跟/反解析）
            final_seq = seq1.copy()
            # 初始化待选取的候选序列池（深拷贝避免原列表被修改）
            remaining_seqs = candidate_seqs.copy()

            while len(final_seq) < 50:
                # 如果剩余可选序列为空，重新填充为全部候选序列（循环选取）
                if not remaining_seqs:
                    remaining_seqs = candidate_seqs.copy()

                # 随机选择一个剩余的候选序列
                random_idx = random.randint(0, len(remaining_seqs) - 1)
                random_seq = remaining_seqs.pop(random_idx)  # 选中后从剩余列表移除

                # 拼接选中的序列（避免一次拼接超出50，仅补充到50为止）
                need_length = 50 - len(final_seq)
                add_seq = random_seq[:need_length]  # 只取需要的长度，防止超出
                final_seq.extend(add_seq)

            # 确保最终序列长度严格为50
            self.bet_sequence = final_seq[:50]

            # 日志：输出原始序列（包含跟/反）
            self.log_signal.emit("success", f"【序列生成】成功生成50个原始投注序列（含跟/反）：{self.bet_sequence}")
            self.log_signal.emit("info",
                                 f"【序列详情】起点（danshuang1）：{seq1} | 后续候选序列数量：{len(candidate_seqs)}个")
    ...
    def fetch_lottery_data(self, TEST_MODE, TARGET_SINGLE, API_URL):
        ...
        if TEST_MODE:
            ...
            msg = f"当前状态：序列索引{self.current_sequence_index}/50（值：{current_seq_value}） | 金额索引{self.current_stake_index} | 当前金额：{current_stake_value} | 初始投注：{init_status} | 待判定投注：{self.current_bet if self.current_bet else '无'} | 上一轮结果：{self.last_round_result} | 上一局开奖：{self.last_actual_lottery_result}"
            ...
            while True:
                ...
                if valid:
                    # 测试模式下记录模拟开奖结果（修复点2：立即更新）
                    self.last_actual_lottery_result = parts[0] if parts else None
                    last_actual_result = self.last_actual_lottery_result
                    self.log_signal.emit("info",
                                         f"测试模式输入结果：{user_input} | 拆分后有效结果：{parts} | 结果类型：{'多值' if len(parts) > 1 else '单值'} | 记录上一局开奖：{self.last_actual_lottery_result}")
                    return user_input
            ...
            try:
                if (danshuang_TT):
                    temp_t = danshuang_TT
                    # 记录实际开奖结果（用于跟/反解析）
                    self.last_actual_lottery_result = temp_t
                    last_actual_result = temp_t
                    danshuang_TT = None
                    return temp_t
                else:
                    return None
    ...
    def wait_until_target_second(self, target_second, TEST_MODE):
        ...
        # 正常模式：精准等待到目标秒数
        bbb = self.load_config()
        CHECK_SECOND = bbb.get("check_second", 10)
        target_second = 10
        ...
        while (True):
            if (time_TT > 50):
                print(f"延迟{target_second}秒")
                time.sleep(target_second)
                return
            else:
                print("还不行，还需要等待啊time_TT=", time_TT)
                time.sleep(2)
    ...
    def get_current_bet_target(self):
        """
        获取当前投注目标（支持解析跟/反为实际单双值）
        修复点3：增强日志和数据校验，确保解析逻辑正确执行
        """
        try:
            ...
            # 获取原始序列值（可能是单/双/跟/反）
            raw_target = self.bet_sequence[self.current_sequence_index]
            self.log_signal.emit("info", f"【原始投注目标】序列索引{self.current_sequence_index} → {raw_target} | 上一局开奖结果：{self.last_actual_lottery_result}")
            print("raw_target=",raw_target)
            # 解析跟/反为实际单双值
            if raw_target in ["跟", "反"]:
                # 检查是否有上一局开奖结果
                if not self.last_actual_lottery_result:
                    self.log_signal.emit("warning", f"无历史开奖结果，无法解析{raw_target}，默认投注单")
                    return "单"
                print("raw_targetzailimiana =", raw_target)
                # 调用解析函数
                actual_target = parse_genfan_to_single_double(raw_target, self.last_actual_lottery_result)
                print("actual_target=",actual_target)
                if not actual_target:
                    self.log_signal.emit("warning", f"解析{raw_target}失败，默认投注单")
                    return "单"

                self.log_signal.emit("success",
                                     f"【解析跟/反】{raw_target}（上局开奖：{self.last_actual_lottery_result}）→ {actual_target}")
                print("actual_target=",actual_target)
    ...
    def execute_betting_logic(self, latest_result, API_URL, CHECK_SECOND, TEST_MODE, TARGET_SINGLE, d, wallet_address,
        ...
        try:
            ...
            msg = f"【当前状态-全量】最新结果: {latest_result}（拆分后：{parts} | 类型：{'多值' if len(parts) > 1 else '单值'}） | 待判定投注：{self.current_bet if self.current_bet else '无'} | 上一轮输赢：{self.last_round_result} | 初始投注：{'已触发' if self.initial_bet_triggered else '未触发（需单值）'} | 序列索引：{self.current_sequence_index}/50（值：{current_seq_value}） | 金额索引：{self.current_stake_index}（值：{current_stake_value}） | 上一局开奖：{self.last_actual_lottery_result}"
            self.log_signal.emit("info", msg)
            ...
            if self.current_bet is not None:
                ...
                self.bet_history.append({
                    ...
                    "last_lottery_result": self.last_actual_lottery_result  # 新增：记录上一局开奖结果
                })
                self.show_bet_statistics()
                self.current_bet = None  # 判定完成后清空待判定投注

            # 3. 新投注生成（核心修改：取预生成的单双序列，支持跟/反解析）
            ...
            # 核心逻辑：区分“初始投注”和“后续投注”
            self.log_signal.emit("info",
                                 f"【投注触发条件】初始投注状态：{self.initial_bet_triggered} | 上一轮输赢：{self.last_round_result} | 结果类型：{'多值' if len(parts) > 1 else '单值'} | 有效结果数：{len(valid_results)} | 上一局开奖：{self.last_actual_lottery_result}")
            ...
            if not self.initial_bet_triggered:
                ...
                if is_single_result:
                    ...
                    self.log_signal.emit("info",
                                         f"  投注目标: {self.current_bet}（序列索引{self.current_sequence_index}，原始值：{self.bet_sequence[self.current_sequence_index]}） | 投注金额: {next_stake}（{self.current_bet}路金额索引{self.current_stake_index}）")
                    # 标记初始投注已触发
    ...
    def show_bet_statistics(self):
        ...
        # 补充当前索引状态+单/双金额+跟/反解析
        current_raw_value = self.bet_sequence[self.current_sequence_index] if self.current_sequence_index < len(
            self.bet_sequence) else "无"
        current_target = self.get_current_bet_target()
        current_stake_value = self.get_current_stake()
        self.log_signal.emit("info",
                             f"当前状态: 序列索引{self.current_sequence_index}/50（原始值：{current_raw_value} → 解析后：{current_target}） | 金额索引：{self.current_stake_index}（{current_target}路金额：{current_stake_value}） | 上一轮输赢: {self.last_round_result} | 上一局开奖：{self.last_actual_lottery_result}")
        self.log_signal.emit("info", "-" * 40)
    ...
    def yewu(self, config_c=None):
        ...
        API_URL = config_c.get("api_url", "")  # page_object
        ...
        self.log_signal.emit("info", "          彩票投注脚本（单/双/跟/反版）")
        ...
        self.log_signal.emit("info", "核心规则（支持单/双/跟/反）:")
        self.log_signal.emit("info", "1. 单双序列：以danshuang1为起点，随机拼接danshuang2-6生成50个值（支持单/双/跟/反）")
        self.log_signal.emit("info", "2. 跟/反规则：跟=上一局结果，反=上一局结果的对立面（如上局单→跟=单，反=双）")
        self.log_signal.emit("info", "3. 金额规则：单路取dan配置，双路取shuang配置，索引越界时自动重置为0")
        self.log_signal.emit("info", "4. 赢局规则：重置序列/金额索引为0 + 重新生成50个单双序列")
        self.log_signal.emit("info", "5. 输局规则：序列/金额索引同步递进（索引越界自动重置）")
        self.log_signal.emit("info", "6. 初始投注：必须单值结果（单/双）才触发")
        self.log_signal.emit("info", "7. 赢局后续：单/多值均投注；输局后续：单/多值均强制投注")
        self.log_signal.emit("info", "判定提示: 赢局 🎉 | 输局 ❌")

        # 测试模式说明（仅测试模式显示，新增跟/反说明）
        if TEST_MODE:
            self.log_signal.emit("info", "\n测试模式说明:")
            ...
            self.log_signal.emit("info", "- 跟/反解析：输入的模拟结果会作为上一局开奖结果，用于解析跟/反")
        ...
        try:
            ...
            while True:
                ...
                if (latest_result == None):
                    continue
                temp1 = ""
                for temp in list(latest_result):
                    temp1 = temp1 + "," + temp
                if (str(temp1).startswith(",")):
                    temp1 = temp1[1:]
                if (str(temp1).endswith(",")):
                    temp1 = temp1[:-1]
                print(temp1)
                latest_result = temp1
                print("latest_result=", latest_result)
